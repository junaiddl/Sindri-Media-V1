...
class Etl:
    def __init__(self):
        ...
        self.matomo_url = "https://matomo.dli.dk/"
        self.readonly_token_auth = "auth"
        # self.superuser_token_auth = "35558210b82b9c941a1dd05e2b796b6a"
        self.superuser_token_auth = "auth"
        self.today = date.today()
        self.yesterday = self.today - timedelta(days=814)        
        self.db_data = []
        self.obj_event = []
        self.skippp = []
        self.mydata = []
    ...
    def get_pages(self,start_date,id):
        ...
        while start_date < self.today:
                ...
                pages_data = []
                URL = f"{self.matomo_url}index.php?module=API&method=Actions.getPageUrls&filter_limit=-1&idSite=3&period=day&date={str(start_date)}&flat=1&format=JSON&token_auth={self.superuser_token_auth}&force_api_session=1"    
                print(URL)
    ...
    def get_newsletter(self,start_date,id):
        try:
            ...
            while start_date < self.today:
                self.compaign_data= []
                # TEMPORARILTY ADDED isSite 3. REVERT IT TO id
                params = {
                        'module': 'API',
    ...
    def get_daily_totals(self,start_date,id):
        ...
        delta = timedelta(days=1)  # Define the step size for the loop, in this case, 1 day
        while start_date < self.today:
            URL = f"{self.matomo_url}index.php?module=API&method=VisitsSummary.get&idSite=3&period=day&date={str(start_date)}&format=JSON&token_auth={self.superuser_token_auth}"
            print(URL)
            data = self.get_data(URL)
            nb_visits = 0
    ...
    def get_uniq_pageviews(self, id,dat):
        URL = f"{self.matomo_url}index.php?module=API&method=Actions.get&idSite=3&period=day&date={str(dat)}&format=JSON&token_auth={self.superuser_token_auth}"
        data = self.get_data(URL)
        for key, val in data.items():
    ...
    def get_events(self,start_date,id):
        ...
        while start_date < self.today:
            obj_arr = []
            URL1 = f"{self.matomo_url}index.php?module=API&method=Events.getCategory&secondaryDimension=eventAction&flat=1&format=JSON&idSite=3&period=day&date={str(start_date)}&expanded=1&token_auth={self.superuser_token_auth}"
            print(URL1)
    ...
    def send_data_quality(self,id, status):
        ...
        try:
            ...
            # cursor, connection = self.get_database_connection()

            # Insert error log into the data_quality_logs table
            insert_query = """
            INSERT INTO prod.posting_status (siteid, ingestion_date, analytics_status)
            VALUES (%s,%s,%s)
            """
    ...
    def truncate_data_quality(self, site):
        ...
        sql_query = f"""DELETE FROM prod.posting_status WHERE ingestion_date = '{current_date}' AND swagger_status IS NULL and siteid = {site}"""
