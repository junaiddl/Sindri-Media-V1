...
def insert_newsletter_data_into_stage_11(cursor, connection, siteid):
    ...
    try:
        ...
        query = f"""INSERT INTO stage.newsletter (referrer_type, referrer_name, site_id, post_id, url, visits, date)
            SELECT a.referrer_type, a.referrer_name, a.site_id, b.ID AS post_id, a.url, a.visits, a.date
            FROM pre_stage.newsletter  a
            LEFT JOIN prod.site_archive_post b ON TRIM(TRAILING '/' FROM a.url) = TRIM(TRAILING '/' FROM b.link)
            WHERE  a.site_id = {siteid};"""
...
def insert_newsletter_data_into_stage_19(cursor, connection, siteid):
    try:
...
def insert_newsletter_data_into_stage_22(cursor, connection, siteid):
    try:
        ...
        query = f"""INSERT INTO stage.newsletter_string (referrer_type, referrer_name, site_id, post_id, url, visits, date)
            SELECT a.referrer_type, a.referrer_name, a.site_id, b.ID AS post_id, a.url, a.visits, a.date
            FROM pre_stage.newsletter a
        ...
        query1 = f"""
            UPDATE stage.newsletter_string
           SET referrer_name = 
            CASE
...
def insert_newsletter_data_into_stage_20(cursor, connection ,siteid):
    print('Data insertion start into Stage')
...
def insert_newsletter_data_into_stage_21(cursor, connection ,siteid):
    print('Data insertion start into Stage')
    try:
        remove_link_slash = f"""
            UPDATE pre_stage.newsletter
