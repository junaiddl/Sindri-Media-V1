    ...
    insert_query = """
     UPDATE pre_stage.site_archive_post_v2
        ...
        CASE Categories
            ...
            WHEN 'news_overview' THEN 'Nyhedsoverblik'
            WHEN 'guide' THEN 'Guide'
            WHEN 'digidoc' THEN 'Digidoc'
...
def update_tags():
   response = requests.get("https://fagbladet3f.dk/wp-json/wp/v2/subject")
   response.raise_for_status()  # Raise an exception for 4xx or 5xx status codes
   ...
   for tag in tags_list:
        ...
        cursor = connection.cursor()
        try:
            update_query= f"update pre_stage.site_archive_post_v2 set Tags = '{tag_name}' where Tags= '{tag_id}' and siteid = 14"
...
def start_execution():
    ...
    try:
        ...
        # update the tag and categories values for site 14
        print("Updating Tags")
        update_tags()
        ...
        insert_into_prod()
        send_data_quality(14, True)
        print("Data Ingestion Into Prod Completed")
    except:
        send_data_quality(14, False)
