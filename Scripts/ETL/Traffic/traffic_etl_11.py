...
class Etl:
    ...
    def insert_data_into_stage(self,siteid,domain):
        ...
        try:
            update_link_query =f"""
                UPDATE pre_stage.traffic_channels_v3
                    SET firsturl = LEFT(firsturl, LENGTH(firsturl) - 1)
                    WHERE firsturl LIKE '%/' AND siteid = {siteid};
            """
            ...
            query = f"""INSERT INTO stage.traffic_channels_v3 (server_date,postid, siteid, date, hour, firsturl, referrer_type, referrer_name, referrer_keyword, batch_id)
                ...
                LEFT JOIN prod.site_archive_post b ON TRIM(TRAILING '/' FROM a.firsturl) = TRIM(TRAILING '/' FROM b.link)
            ...
            try:
                self.connection_db() 
                self.cursor.execute(update_link_query)
                self.connection_db() 
    ...
    def start_execution(self):
        ...
        for id in [11]:
            try:
                ...
                self.connection_db()
                self.send_data_quality(11, True)
                self.connection.commit()
                self.cursor.close()
                self.connection.close()
            except mysql.connector.Error as error:
                self.send_data_quality(11, False)
                print("Error inserting data into MySQL database:", error)     
