...
class Etl:
    ...
    def truncate(self,site):
        try:
            ...
            self.connection.commit()
            truncate_query = f"DELETE FROM stage.traffic_channels_v3_string WHERE siteid = {site};"
            self.cursor.execute(truncate_query)
            self.connection.commit()
            self.cursor.close()
            self.connection.close()
    ...
    def insert_data_into_stage(self,siteid,domain):
        ...
        try:
            query = f"""INSERT INTO stage.traffic_channels_v3_string (server_date,postid, siteid, date, hour, firsturl, referrer_type, referrer_name, referrer_keyword, batch_id)
                        ...
                        LEFT JOIN prod.site_archive_post_string b ON  a.firsturl = b.Link
            ...
            update_link_query14 =f"""
                UPDATE pre_stage.traffic_channels_v3 a
                        ...
                        WHERE a.SITEID = {siteid}
            """
            print('insert_data_into_stage for site 20')
            query1 = f"""
                UPDATE stage.traffic_channels_v3_string
                SET RealReferrer = 
                    CASE
                         -- Direct
    ...
    def insert_data_into_prod(self,siteid):
        ...
        print('insert into entry pages for site')
        query = f"""INSERT INTO prod.entry_pages_agg (Siteid, Date, RealReferrer, referrer_name, referrer_keyword, firsturl, hits)
                        WITH conditions AS (
                            SELECT
                                ...
                                firsturl
                                FROM stage.traffic_channels_v3_string
                            WHERE
                                Siteid = {siteid}
                        )
        ...
        print('delete visits rythm for sites')
        ...
        query2 = f"""INSERT INTO  prod.realreferrer (Siteid, Date, RealReferrer, Hits)
                    ...
                    FROM   stage.traffic_channels_v3_string where siteid = {siteid}
                    GROUP BY Siteid, Date, RealReferrer;
        """

        query3 = f"""
            INSERT INTO prod.traffic_channels_string (SiteID, postid , date, hour, firsturl, referrer_type, referrer_name, referrer_keyword, RealReferrer, batch_id )
            SELECT SiteID, postid , date, hour, firsturl, referrer_type, referrer_name, referrer_keyword, RealReferrer, batch_id FROM stage.traffic_channels_v3_string
            where siteid = {siteid};
        """

        query3_delete = f""" delete  from prod.traffic_channels_string  where siteid = {siteid} and date = '{self.yesterday}';"""
