...
class Etl:
    def __init__(self):
        # self.matomo_url = "https://soclan.matomo.cloud/index.php?"
        # self.token_auth = "3aa47636bc67b03d21186aba44284955"
        self.matomo_url = "https://pubmetrics.online/index.php?"
        self.token_auth = "auth"
        self.today =  date.today()
        self.yesterday = self.today - timedelta(days=4)
        self.cursor = None
            ...
            while start_date < self.today:  
                ...
                try:
                    for item in all_data:
                        ...
                        # if 'actionDetails' in item and isinstance(item['actionDetails'], list) and len(item['actionDetails']) > 0:
                        if item is not None and len(item) > 0:
                            try:
                                ...
                                first_URL = str(parsed_url.scheme) + '://' + str(parsed_url.netloc) + str(parsed_url.path)
                    ...
                    if len(self.compaign_data) > 0:
                        print(len(self.compaign_data))
                        self.insert_compaign(self.compaign_data)
                    else:
                        print('lenght of compaign_data is less')
                
                except Exception as e:
                    print("An error occurred: at this point", str(e))  
                   
                

        except Exception as e:
            print("An error occurred: 28", str(e))
    ...
    def insert_compaign(self, data):
        print('insert data to pre stage traffic_channels_v3 ')
        try:
            total = len(data)
            count = 0
            failed = 0
            ...
            query = "INSERT INTO pre_stage.traffic_channels_v3 (siteid, server_date, firsturl, referrer_type, referrer_name, referrer_keyword, date, hour, batch_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s,%s)"
                ...
                CASE
                    ...
                    -- Newsletter
                        ...
                        WHEN referrer_type = 'campaign' AND referrer_name = 'mail-c'
                            THEN 'NL Leder'
                        WHEN referrer_type = 'campaign' AND referrer_name = 'mail-d'
                            THEN 'NL Pleje'
	...
	WITH conditions AS (
        SELECT
            ...
            CASE 
                ...
                WHEN referrer_type = 'campaign' AND referrer_name = 'mail-d' THEN referrer_name
				ELSE NULL
            ...
            CASE 
                ...
                WHEN referrer_type = 'campaign' AND referrer_name = 'mail-d' THEN referrer_keyword
      ...
      ;"""
        ...
        query2 = f"""INSERT INTO  prod.realreferrer (Siteid, Date, RealReferrer, Hits)
            SELECT 15 as Siteid, Date, RealReferrer, COUNT(*) as hits
    ...
    def start_execution(self):
        for id in [15]:
            try:
                ...
                # exit()
                self.truncate(15)
                self.truncate_data_quality(15)
                self.connection_db()
                self.get_compaign(self.yesterday,id)
                self.connection_db()
                query_do = f"SELECT domain FROM  prod.domain_siteid  where siteid = 15"
                self.cursor.execute(query_do)
                rows = self.cursor.fetchall()
                domain = rows[0][0]
                print('Domain: ',domain)
                self.insert_data_into_stage(15,domain)   
                self.insert_data_into_prod(15)
                self.send_data_quality(15, True)
                self.connection.commit()
            except mysql.connector.Error as error:
                self.send_data_quality(15, False)
                print("Error inserting data into MySQL database:", error)     
