...
CREATE PROCEDURE `tactical_userneeds_pageviews_chart_ytd_dbt_4`()
BEGIN
    with cms_data as (
	...
	agg_next_click_per_article_without_others as(
                ...
                SELECT siteid, id, fdate, "Leder" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Leder.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Opinion" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Opinion.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Analyse" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Analyse.*"
                UNION ALL
	...
	counts as (
		...
		categories_d as (
			select 
                ...
                'Artikler publiceret år til dato grupperet i tre kategorier: Artikler under sign-ups mål | Artikler, der nåede sign-ups mål | Top 10% bedste artikler ift. sign-ups. For hver kategori vises hvor stor andel der er publiceret indenfor hvert tag, så det er muligt at se forskelle på tværs af kategorierne.' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Nyheder', 'Leder', 'Opinion', 'Analyse', 'Reportage', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Nyheder', 'Leder', 'Opinion', 'Analyse', 'Reportage', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Nyheder', 'Leder', 'Opinion', 'Analyse', 'Reportage', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Nyheder', 'Leder', 'Opinion', 'Analyse', 'Reportage', 'others' ) SEPARATOR ',') AS top_10
		from agg_data
		group by 1,2,3
		),
        categories as (
