        ...
        CREATE PROCEDURE `performance_articles0_card_day_dbt_4`()
        BEGIN
        ...
        and e.date between DATE_SUB(NOW(), INTERVAL 8 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY)  and e.siteid = 4
        ...
        left join prod.events e
        on e.postid=ldp.id 
        and e.date between DATE_SUB(NOW(), INTERVAL 15 DAY) and DATE_SUB(NOW(), INTERVAL  8 DAY)  and e.siteid = 4
        and e.Event_Action = 'Newsletter'
        group by 1,2,3,5
        ),
        ...
        case when coalesce(l.hits,0)<coalesce(g.Min_CTA,0)  and coalesce(l.pageviews,0)<coalesce(g.Min_pageviews,0) then 1 else 0 end as gt_goal  from last_7days_hits_pages l
  ...
  IFNULL((
    SELECT JSON_OBJECT(
      'data', JSON_OBJECT(
        ...
        'value', IFNULL(lg.value, 0),
        'change', IFNULL(ROUND(((lg.value - pg.value) / NULLIF(pg.value, 0)) * 100, 2), 0),
        'progressCurrent', 0,
        'progressTotal', 0
      ),
      'site', lg.siteid
  ...
  JSON_OBJECT(
    ...
    'site', 4
        ...
        --               'hint',  "Artikler publiceret seneste 7 dage, der er under på begge mål",
        --               'value', IFNULL(lg.value, 0),
        --               'change', IFNULL(coalesce(round(((lg.value-pg.value)/pg.value)*100,2),0), 0),
        --               "progressCurrent", 0,
        --         "progressTotal", 0
        --           ),
        --           'site', lg.siteid
