...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_4`(
    IN site INT,
...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        ...
        WHERE realreferrer IN ( ''Forside'',''Search'', ''Facebook'',''X'',''Newsletter'' ) and siteid = 4'
    );
    
        SET @categories_sql_first = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Nyheder'',''Leder'',''Opinion'',''Analyse'',''Reportage'' ) and siteid = 4'
        );
    
        SET @categories_sql_second = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Opdater mig'',''Forbind mig'',''Hjælp mig med at forstå'',''Giv mig en fordel'',''Underhold mig'',''Inspirer mig'' ) and siteid = 4'
        );
    ...
    SET @sql_query = CONCAT('
        ...
        pageview_first AS (
            ...
            WHERE siteid = 4 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
                AND r.realreferrer IN ( ''Forside'',''Search'', ''Facebook'',''X'' )
        ...
        main_first AS (
            ...
            FROM prod.events
            WHERE siteid = 4 AND Event_Action = ''Frontpage'' 
        ...
        transformed_data_first AS (
            SELECT
                ...
                t_totals,
                CASE 
                    WHEN event_name LIKE ''://%'' THEN CONCAT(''https://piopio.dk/'', SUBSTRING(event_name, 5))
                    ELSE event_name
                END AS modifyurl
            FROM main_first
        ),
        cms_groups_without_others_first as(
            ...
            SELECT siteid, id, date, "Leder" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Leder.*"
            UNION ALL
            
            SELECT siteid, id, date, "Opinion" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Opinion.*"
            UNION ALL
            
            SELECT siteid, id, date, "Analyse" AS tags, link
            FROM cms_data
