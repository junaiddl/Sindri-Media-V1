        ...
        CREATE PROCEDURE `performance_articles2_card_ytd_dbt_4`()
        BEGIN
            ...
            join prod.pages e
            on e.postid=ldp.id and e.siteid = ldp.siteid
            where e.date  between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY) and e.siteid = 4
            group by 1,2,3
            ),
