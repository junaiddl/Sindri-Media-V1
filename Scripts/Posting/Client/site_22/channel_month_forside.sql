...
CREATE DEFINER=`Site`@`%` PROCEDURE `Traffic_channel_forside_month_dbt_22`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
		...
		card_visits_last_year AS (
		 ...
		 final_card as(
			...
			COALESCE(cur_v.siteid, 22) AS siteid,
			...
			FROM (SELECT 22 AS siteid, ''Forside'' AS realreferrer, 0 AS visits) dummy
		...
		channels_table_real AS (
			...
			FROM prod.events_string
			WHERE
				siteid = 22 and event_action = ''Frontpage'' and
 ...
 AS Categories,
			...
			WHERE s.siteid = 22 and r.realreferrer = ''Forside''
...
''sektion'', coalesce(Categories, " "),
       ...
       visits_per_dayname AS (
		...
		Result AS (
			SELECT
				COALESCE(lastyear.siteid,22) as siteid,
				ah.series,
