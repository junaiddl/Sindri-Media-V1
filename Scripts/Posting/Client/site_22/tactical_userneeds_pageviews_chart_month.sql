...
CREATE PROCEDURE `tactical_userneeds_pageviews_chart_month_dbt_22`()
BEGIN
	with cms_data as (
	...
	counts as (
		...
		last_30_days_article_published_without_others_second as(
                SELECT siteid, id, fdate, "Evergreen" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Evergreen.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Testartikel" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Testartikel.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Redaktionelt" AS tags
                FROM cms_data
	...
	counts_second as (
		...
		categories_d_second as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Evergreen','Testartikel','Redaktionelt','Andet', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Evergreen','Testartikel','Redaktionelt','Andet', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Evergreen','Testartikel','Redaktionelt','Andet', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Evergreen','Testartikel','Redaktionelt','Andet', 'others' ) SEPARATOR ',') AS top_10
		...
		last_30_days_article_published_without_others_third as(
                SELECT siteid, id, fdate, "Forbrugerliv" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Forbrugerliv.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Test" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Test.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Kemi" AS tags
                FROM cms_data
                ...
                SELECT siteid, id, fdate, "Rådgivning" AS tags
                FROM cms_data
	...
	counts_third as (
		...
		categories_d_third as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Forbrugerliv','Test','Kemi','Privatøkonomi','Rådgivning', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Forbrugerliv','Test','Kemi','Privatøkonomi','Rådgivning', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Forbrugerliv','Test','Kemi','Privatøkonomi','Rådgivning', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Forbrugerliv','Test','Kemi','Privatøkonomi','Rådgivning', 'others' ) SEPARATOR ',') AS top_10
		from agg_data_third
		group by 1,2,3
		),
        categories_third as (
