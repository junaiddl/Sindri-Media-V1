...
CREATE PROCEDURE `Traffic_channel_week_dbt_22`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        card_visits AS (
			...
			FROM referrer rc
			LEFT JOIN prod.traffic_channels_string tc ON
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id","label": "ID"},{"field": "article","label": "ARTIKEL"},{"field": "category","label": "BRUGERBEHOV"},{"field": "sektion", "label": "Artikeltype", "hidden": true},{"field": "tags", "label": "Sektion", "hidden": true},{"field": "date","label": "DATO"},{"field": "brugerbehov","label": "Besøg"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
