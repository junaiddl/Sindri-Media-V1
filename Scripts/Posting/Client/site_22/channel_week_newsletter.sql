...
CREATE PROCEDURE `Traffic_channel_week_newsletter_dbt_22`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        card_visits AS (
			...
			FROM referrer rc
			LEFT JOIN prod.newsletter_string n ON
 ...
 AS Categories,
			...
			LEFT JOIN prod.site_archive_post_string s ON  r.postid = s.id  and r.siteid = s.siteid
			WHERE s.siteid = 22 and r.realreferrer = ',channel_label,'
...
''sektion'', coalesce(Categories, " "),
		...
		WeekVisits_chartbar AS (
			SELECT
				...
				ad.day_order
			FROM AllDays ad
			LEFT JOIN prod.traffic_channels_string tc ON ad.english_day = DAYNAME(tc.`date`)
				AND date BETWEEN  DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY)
				AND siteid = 22 and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		...
		LastWeekVisits AS (
			...
			FROM prod.traffic_channels_string
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,22) as siteid,
				ah.series,
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id","label": "ID"},{"field": "article","label": "ARTIKEL"},{"field": "category","label": "BRUGERBEHOV"},{"field": "sektion", "label": "Artikeltype", "hidden": true},{"field": "tags", "label": "Sektion", "hidden": true},{"field": "date","label": "DATO"},{"field": "brugerbehov","label": "Kliks"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
