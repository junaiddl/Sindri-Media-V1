      ...
      CREATE  PROCEDURE `overview_visits_chart_month_dbt_19`()
      BEGIN

      SET SESSION group_concat_max_len = 10000;
      WITH agg_on_month as (
        SELECT SiteID,month(date) as month,DATE_FORMAT(date,'%b') as monthname,SUM(Visits) as data
        FROM prod.daily_totals
        where date
        between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY) and siteid= 19
        group by 1,2,3
      ),
      agg_goal as(
        select site_id,month(date) as month,DATE_FORMAT(date,'%b') as monthname,sum(Visits_per_day) AS goals from prod.goals
          where site_id =  19 and YEAR(date) = YEAR(CURRENT_DATE())
        group by 1,2,3
      ),
      agg_prev_year AS (
        SELECT SiteID, month(date) AS month,DATE_FORMAT(date,'%b') as monthname, COALESCE(SUM(Visits), 0) AS pre_year
      ...
      json_data_prev as( 
        SELECT
        COALESCE(ap.SiteID,ag.site_id) as SiteID,'Besøg' as label,'Besøg år til dato grupperet på hhv. dag, uge og måned'  as hint ,
        ...
        group by COALESCE(ap.SiteID,ag.site_id),label,hint
      ),
      json_data_curr as( 
        SELECT
        COALESCE(ag.site_id,ao.SiteId) as SiteID,'Besøg' as label,'Besøg år til dato grupperet på hhv. dag, uge og måned'  as hint ,
        GROUP_CONCAT( concat('"',ag.monthname,'"')  order by ag.month  SEPARATOR ', ') AS categories,
