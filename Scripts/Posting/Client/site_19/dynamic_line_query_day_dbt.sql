...
CREATE PROCEDURE `Traffic_dynamic_linequery_day_dbt_19`(
	IN site INT,
...
BEGIN
   SET SESSION group_concat_max_len = 100000;
    ...
    SET @referrers_sql = CONCAT(
        'SELECT DISTINCT realreferrer FROM pre_stage.ref_value WHERE realreferrer IN ( ''Direct'', ''Search'', ''LinkedIn'', ''Newsletter'' ) and siteid = 19'
    );
    ...
    SET @sql_query = CONCAT('
		...
		agg_on_day AS (
			...
			LEFT JOIN prod.traffic_channels t ON d.date = t.date AND t.siteid = 19
		...
		final_ar as (
			...
			''Besøg'' as label, ''Besøg opdelt på trafikkanaler'' as hint,
			  categories AS categories,
				GROUP_CONCAT(realreferrer ORDER BY FIELD(LOWER(realreferrer),  ''Direct'', ''Search'', ''LinkedIn'', ''Newsletter'' , ''others'' ) SEPARATOR '','') AS series,
			  GROUP_CONCAT(referrer_counts ORDER BY FIELD(LOWER(realreferrer),  ''Direct'', ''Search'', ''LinkedIn'', ''Newsletter'' , ''others'') SEPARATOR '','') AS data
			FROM json_data
			GROUP BY categories
		)
        SELECT
