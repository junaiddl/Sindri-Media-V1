...
CREATE PROCEDURE `bigview_header_yesterday_dbt_19`()
BEGIN
...
unique_pageviews_card AS (
    SELECT 19 as siteid, y.value, b.value_day_before
...
events_yesterday as (
    ...
    WHERE
    e.siteid = 19 and
        e.date = DATE_SUB(CURDATE(), INTERVAL 1 DAY)
...
events_day_before as (
    ...
    FROM
        prod.events e
    WHERE
    e.date = DATE_SUB(CURDATE(), INTERVAL 2 DAY)
    and e.siteid = 19
    AND e.event_action = 'Next Click'
    group by SiteId,2
    ),
...
pages_yesterday as (
    ...
    WHERE
    p.siteid = 19 and
        p.date = DATE_SUB(CURDATE(), INTERVAL 1 DAY)
    GROUP BY SiteId,2
    ),
pages_day_before as (
    SELECT
    ...
    FROM
        prod.pages p
    WHERE
    p.date = DATE_SUB(CURDATE(), INTERVAL 2 DAY)
    and p.siteid = 19
    group by SiteId,2
    ),
...
combined_ratios AS (
    ...
    LEFT JOIN pages_yesterday p ON e.SiteID = p.SiteID
    LEFT JOIN events_day_before e_day_before ON e.SiteID = e_day_before.SiteID
    LEFT JOIN pages_day_before p_day_before ON p.SiteID = p_day_before.SiteID
    ),

next_click_card as (
    select  siteid, ROUND((sum(ratio_yesterday)/count(*) * 100),1) as yr_final, ROUND((sum(ratio_day_before)/count(*) * 100),1) as rd_final
...
unique_pageviews_chart as (
    ...
    where date BETWEEN DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY) and siteid = 19
    group by 1,2
    ),
...
visits_chart as (
    ...
    FROM prod.daily_totals
    where
    date BETWEEN DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY) and siteid = 19
    group by 1,2
    ),
...
next_click_chart AS(
    SELECT 19 AS siteid, e.date AS date, ROUND((SUM(e.data)/SUM(p.sum_of_pageviews))*100,1) AS next_click_data
    FROM events_chart e
