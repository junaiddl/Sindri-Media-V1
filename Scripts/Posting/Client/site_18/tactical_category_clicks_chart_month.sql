...
CREATE PROCEDURE `tactical_category_clicks_chart_month_dbt_18`()
BEGIN
    with 
    ...
    agg_data as (
        ...
        where g.date  between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY)  and g.site_id =  18
    ),
    data_out as(
    select 
        a.siteid as siteid,
