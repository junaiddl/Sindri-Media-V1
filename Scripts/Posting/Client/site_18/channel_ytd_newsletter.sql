...
CREATE PROCEDURE `Traffic_channel_ytd_newsletter_dbt_18`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
		...
		channels_table_real AS (
			...
			WHERE
				...
				date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()) - 1, 1)
                            AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
			GROUP BY 1, 2, 3, 4
		),
		site_j AS (
 ...
 AS Categories,
			...
			WHERE s.siteid = 18 and r.realreferrer = ',channel_label,'
...
''sektion'', coalesce(Categories, " "),
        ...
        visits_per_dayname AS (
			...
			LEFT JOIN prod.traffic_channels tc ON ad.english_day = DAYNAME(tc.`date`)
				...
				AND siteid = 18 and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,18) as siteid,
				ah.series,
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id","label": "ID"},{"field": "article","label": "ARTIKEL"},{"field": "category","label": "BRUGERBEHOV"},{"field": "sektion","label": "KATEGORI","hidden": true},{"field": "tags","label": "TAGS","hidden": true},{"field": "date","label": "DATO"},{"field": "brugerbehov","label": "Kliks"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
