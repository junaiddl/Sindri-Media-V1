...
CREATE PROCEDURE `Traffic_channel_ytd_newsletter_dbt_21`(
    IN site INT,
...
BEGIN
 ...
 AS Categories,
			...
			WHERE s.siteid = 21 and r.realreferrer = ',channel_label,'
...
''sektion'', coalesce(Categories, " "),
        ...
        visits_per_dayname AS (
			...
			LEFT JOIN prod.traffic_channels tc ON ad.english_day = DAYNAME(tc.`date`)
				...
				AND siteid = 21 and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,21) as siteid,
				ah.series,
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id","label": "ID"},{"field": "article","label": "ARTIKEL"},{"field": "category","label": "BRUGERBEHOV"},{"field": "sektion", "label": "Kategori", "hidden": true},{"field": "tags", "label": "Artikeltype", "hidden": true},{"field": "date","label": "DATO"},{"field": "brugerbehov","label": "Kliks"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
