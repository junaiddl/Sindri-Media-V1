...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_21`(
    IN site INT,
...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        ...
        WHERE realreferrer IN ( ''Forside'',''Search'',''Facebook'',''NL'' ) and siteid = 21'
    );
    
        SET @categories_sql_first = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Opdater mig'',''Forbind mig'',''Hjælp mig med at forstå'',''Giv mig en fordel'',''Underhold mig'',''Inspirer mig'' ) and siteid = 21'
        );
    
        SET @categories_sql_second = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Jura'',''Historie'',''Guide'',''Konfliktmægler'' ) and siteid = 21'
        );
    
        SET @categories_sql_third = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Brevkasse'',''Article'' ) and siteid = 21'
        );
    ...
    SET @sql_query = CONCAT('
        ...
        combinations_first AS (
            ...
            ORDER BY FIELD(r.realreferrer, ''Forside'',''Search'',''Facebook'',''NL'', "Others")
        ),
        pageview_first AS (
            SELECT 
                siteid,
            ...
            WHERE siteid = 21 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
        ...
        main_first AS (
            ...
            FROM prod.events
            WHERE siteid = 21 AND Event_Action = ''Frontpage'' 
        ...
        transformed_data_first AS (
            SELECT
                ...
                t_totals,
                CASE 
                    WHEN event_name LIKE ''://%'' THEN CONCAT(''https://bogodtbladet.dk/'', SUBSTRING(event_name, 5))
                    ELSE event_name
                END AS modifyurl
            FROM main_first
        ),
        ...
        summed_data_first AS (
			...
			ORDER BY FIELD(c.realreferrer, ''Forside'',''Search'',''Facebook'',''NL'', "Others")
        ),
        percent_first AS (
            SELECT
                realreferrer,
        ...
        pivoted_data_first AS (
			...
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''Facebook'',''NL'', "Others")
		),
        final_first as(
			SELECT 	
        ...
        cms_groups_without_others_second as(
            SELECT siteid, id, date, "Jura" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Jura.*"
            UNION ALL
            
            SELECT siteid, id, date, "Historie" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Historie.*"
            UNION ALL
            ...
            SELECT siteid, id, date, "Konfliktmægler" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Konfliktmægler.*"
        ),
        ...
        pivoted_data_second AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''Jura'',''Historie'',''Guide'',''Konfliktmægler'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''Jura'',''Historie'',''Guide'',''Konfliktmægler'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''Jura'',''Historie'',''Guide'',''Konfliktmægler'', "Others")), '']'') AS percentile
			FROM percent_second
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''Facebook'',''NL'', "Others")
		),
        ...
        cms_groups_without_others_third as(
            SELECT siteid, id, date, "Brevkasse" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Brevkasse.*"
            UNION ALL
            
            SELECT siteid, id, date, "Article" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Article.*"
        ),
        cms_groups_third as (
            SELECT a.siteid, a.id, a.date, ''Others'' as tags, a.link
            FROM cms_data a
        ...
        pivoted_data_third AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''Brevkasse'',''Article'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''Brevkasse'',''Article'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''Brevkasse'',''Article'', "Others")), '']'') AS percentile
			FROM percent_third
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''Facebook'',''NL'', "Others")
		),
