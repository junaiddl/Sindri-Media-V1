...
CREATE PROCEDURE `bigview_header_yesterday_dbt_21`()
BEGIN
...
unique_pageviews_card AS (
    SELECT 21 as siteid, y.value, b.value_day_before
...
unique_pageviews_chart as (
    ...
    where date BETWEEN DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY) and siteid = 21
    group by 1,2
    ),
...
visits_chart as (
    ...
    FROM prod.daily_totals
    where
    date BETWEEN DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY) and siteid = 21
    group by 1,2
    ),
...
next_click_chart AS(
    SELECT 21 AS siteid, e.date AS date, ROUND((SUM(e.data)/SUM(p.sum_of_pageviews))*100,1) AS next_click_data
    FROM events_chart e
