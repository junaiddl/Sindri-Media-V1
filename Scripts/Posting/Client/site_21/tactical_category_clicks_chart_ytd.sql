...
CREATE PROCEDURE `tactical_category_clicks_chart_ytd_dbt_21`()
BEGIN
    with last_ytd_article_published as (
        SELECT siteid as siteid,id,date as fdate   FROM prod.site_archive_post
        where 
        date  between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)  and siteid =21
    ...
    agg_data as ( 
        ...
        where  g.date  between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)  and g.site_id = 21
    ),
    data_out as (
        select 
        a.siteid as siteid,
