...
CREATE PROCEDURE `Traffic_pageview_month_dbt_13`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        pageview_first AS (
            ...
            WHERE siteid = 13 AND date between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY)
        ...
        newsletter_data_first AS (
            SELECT 
                ...
                AND n.referrer_name IN ( ''NL Medlem'', ''NL fra NNF'' )
        ...
        cms_groups_without_others_second as(
            ...
            SELECT siteid, id, date, "Medlemsinfo" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Medlemsinfo.*"
            UNION ALL
            
            SELECT siteid, id, date, "Reportage" AS tags, link
            FROM cms_data
            ...
            SELECT siteid, id, date, "Det ku'' ske for dig" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Det ku'' ske for dig.*"
            UNION ALL
            
            SELECT siteid, id, date, "Jeg mener" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Jeg mener.*"
            UNION ALL
            
            SELECT siteid, id, date, "Videoartikel" AS tags, link
            FROM cms_data
        ...
        cms_groups_without_others_third as(
            SELECT siteid, id, date, "Slagterindustri" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Slagterindustri.*"
            UNION ALL
            
            SELECT siteid, id, date, "Fødevareindustri" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Fødevareindustri.*"
            UNION ALL
            
            SELECT siteid, id, date, "Mejeri" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Mejeri.*"
            UNION ALL
            
            SELECT siteid, id, date, "Butik" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Butik.*"
            UNION ALL
            
            SELECT siteid, id, date, "Alle brancher" AS tags, link
            FROM cms_data
