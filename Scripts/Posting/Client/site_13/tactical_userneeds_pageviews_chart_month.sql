...
CREATE PROCEDURE `tactical_userneeds_pageviews_chart_month_dbt_13`()
BEGIN
	with cms_data as (
	...
	counts as (
		...
		last_30_days_article_published_without_others_second as(
                ...
                FROM cms_data
                WHERE Categories REGEXP ".*Nyhed.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Medlemsinfo" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Medlemsinfo.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Reportage" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Reportage.*"
                UNION ALL
                ...
                SELECT siteid, id, fdate, "Det ku' ske for dig" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Det ku' ske for dig.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Jeg mener" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Jeg mener.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Videoartikel" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Videoartikel.*" 
        ),
	...
	counts_second as (
		...
		categories_d_second as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Nyhed','Medlemsinfo','Reportage','Artikel','Det ku'' ske for dig','Jeg mener','Videoartikel', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Nyhed','Medlemsinfo','Reportage','Artikel','Det ku'' ske for dig','Jeg mener','Videoartikel', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Nyhed','Medlemsinfo','Reportage','Artikel','Det ku'' ske for dig','Jeg mener','Videoartikel', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Nyhed','Medlemsinfo','Reportage','Artikel','Det ku'' ske for dig','Jeg mener','Videoartikel', 'others' ) SEPARATOR ',') AS top_10
		...
		last_30_days_article_published_without_others_third as(
                SELECT siteid, id, fdate, "Slagterindustri" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Slagterindustri.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Fødevareindustri" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Fødevareindustri.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Mejeri" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Mejeri.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Butik" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Butik.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Alle brancher" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Alle brancher.*" 
        ),
	...
	counts_third as (
		...
		categories_d_third as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Slagterindustri','Fødevareindustri','Mejeri','Butik','Alle brancher', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Slagterindustri','Fødevareindustri','Mejeri','Butik','Alle brancher', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Slagterindustri','Fødevareindustri','Mejeri','Butik','Alle brancher', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Slagterindustri','Fødevareindustri','Mejeri','Butik','Alle brancher', 'others' ) SEPARATOR ',') AS top_10
		from agg_data_third
		group by 1,2,3
		),
        categories_third as (
