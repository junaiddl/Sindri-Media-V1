    ...
    CREATE  PROCEDURE `overview_pageviews_card_dbt_13`()
    BEGIN
    with curr_year as (
    SELECT
    ...
    FROM
        prod.goals
    where  Site_ID= 13 and date
    between (SELECT MIN(date) AS MinimumDate
        FROM prod.daily_totals
        WHERE siteid =13 and YEAR(date) = YEAR(CURRENT_DATE()))  and  (SELECT MAX(date) AS MaximumDate
                    FROM prod.daily_totals
                    WHERE siteid = 13 and YEAR(date) = YEAR(CURRENT_DATE()))
    group by Site_Id
    ),
    json_data as( 
