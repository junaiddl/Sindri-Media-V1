...
CREATE PROCEDURE `Traffic_channel_ytd_newsletter_dbt_13`(
    IN site INT,
...
BEGIN
 ...
 AS Categories,
			...
			WHERE s.siteid = 13 and r.realreferrer = ',channel_label,'
...
''sektion'', coalesce(Categories, " "),
        ...
        visits_per_dayname AS (
			...
			LEFT JOIN prod.traffic_channels tc ON ad.english_day = DAYNAME(tc.`date`)
				...
				AND siteid = 13 and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,13) as siteid,
				ah.series,
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id", "label": "ID"}, {"field": "article", "label": "ARTIKEL"}, {"field": "category", "label": "BRUGERBEHOV"}, {"field": "sektion", "label": "TYPE", "hidden": true}, {"field": "tags", "label": "Tags", "hidden": true}, {"field": "date", "label": "DATO"},{"field": "brugerbehov","label": "Kliks"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
