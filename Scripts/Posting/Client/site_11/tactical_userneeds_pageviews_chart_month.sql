...
CREATE PROCEDURE `tactical_userneeds_pageviews_chart_month_dbt_11`()
BEGIN
	with cms_data as (
	...
	counts_second as (
		...
		categories_d_second as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Artikel', 'Seneste nyt', 'Guide', 'Debat', 'Feature', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Artikel', 'Seneste nyt', 'Guide', 'Debat', 'Feature', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Artikel', 'Seneste nyt', 'Guide', 'Debat', 'Feature', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Artikel', 'Seneste nyt', 'Guide', 'Debat', 'Feature', 'others' ) SEPARATOR ',') AS top_10
		...
		last_30_days_article_published_without_others_third as(
                SELECT siteid, id, fdate, "Billedkunst" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Billedkunst.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Børnehaveklassen" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Børnehaveklassen.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Dansk" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Dansk.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Dansk som andetsprog" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Dansk som andetsprog.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "DSA" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*DSA.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Engelsk" AS tags
                FROM cms_data
                ...
                SELECT siteid, id, fdate, "Idræt" AS tags
                FROM cms_data
                ...
                SELECT siteid, id, fdate, "Kulturfag" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Kulturfag.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Madkundskab" AS tags
                FROM cms_data
                ...
                SELECT siteid, id, fdate, "Naturfag" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Naturfag.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "PLC" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*PLC.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "PPR" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*PPR.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Praktik" AS tags
                FROM cms_data
                ...
                SELECT siteid, id, fdate, "Sosu-undervisere" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Sosu-undervisere.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Specialpædagogik" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Specialpædagogik.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Studie" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Studie.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Tysk og fransk" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Tysk og fransk.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Uddannelsesvejledning" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Uddannelsesvejledning.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "UU" AS tags
                FROM cms_data
	...
	counts_third as (
		...
		categories_d_third as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Billedkunst','Børnehaveklassen','Dansk','Dansk som andetsprog','DSA','Engelsk','Håndværk og design','Idræt','It','Kulturfag','Madkundskab','Matematik','Musik','Naturfag','PLC','PPR','Praktik','Senior','SOSU','Sosu-undervisere','Specialpædagogik','Studie','Tysk og fransk','Uddannelsesvejledning','UU', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Billedkunst','Børnehaveklassen','Dansk','Dansk som andetsprog','DSA','Engelsk','Håndværk og design','Idræt','It','Kulturfag','Madkundskab','Matematik','Musik','Naturfag','PLC','PPR','Praktik','Senior','SOSU','Sosu-undervisere','Specialpædagogik','Studie','Tysk og fransk','Uddannelsesvejledning','UU', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Billedkunst','Børnehaveklassen','Dansk','Dansk som andetsprog','DSA','Engelsk','Håndværk og design','Idræt','It','Kulturfag','Madkundskab','Matematik','Musik','Naturfag','PLC','PPR','Praktik','Senior','SOSU','Sosu-undervisere','Specialpædagogik','Studie','Tysk og fransk','Uddannelsesvejledning','UU', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Billedkunst','Børnehaveklassen','Dansk','Dansk som andetsprog','DSA','Engelsk','Håndværk og design','Idræt','It','Kulturfag','Madkundskab','Matematik','Musik','Naturfag','PLC','PPR','Praktik','Senior','SOSU','Sosu-undervisere','Specialpædagogik','Studie','Tysk og fransk','Uddannelsesvejledning','UU', 'others' ) SEPARATOR ',') AS top_10
		from agg_data_third
		group by 1,2,3
		),
        categories_third as (
