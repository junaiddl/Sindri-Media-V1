...
CREATE PROCEDURE `Traffic_dynamic_linequery_month_dbt_11`(
    IN site INT,
...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        'SELECT DISTINCT realreferrer FROM pre_stage.ref_value WHERE realreferrer IN ( ''Direct'', ''Search'', ''Facebook'', ''Newsletter'', ''NL Red'', ''NL Fag'', ''NL Dag'',''NL DLF'' ,''NL Stud''  ) and siteid = 11 '
    );
    ...
    SET @sql_query = CONCAT('
		...
		agg_on_day AS (
			...
			LEFT JOIN prod.traffic_channels t ON d.month_name = DATE_FORMAT(t.date, ''%Y-%m'') AND t.siteid = 11
