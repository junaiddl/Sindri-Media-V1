    ...
    CREATE  PROCEDURE `overview_pageviews_chart_day_dbt_10`()
    BEGIN
	...
	agg_goal AS (
		SELECT site_id, date, SUM(Pageviews_per_day) AS goals
		FROM prod.goals
		WHERE site_id = 10 AND YEAR(date) = YEAR(CURRENT_DATE())
		GROUP BY 1, 2
	),
	agg_prev_year AS (
		SELECT SiteID, date AS date, SUM(Unique_pageviews) AS pre_year
		FROM prod.daily_totals dt
		WHERE siteid = 10 AND YEAR(date) = YEAR(CURRENT_DATE()) - 1
