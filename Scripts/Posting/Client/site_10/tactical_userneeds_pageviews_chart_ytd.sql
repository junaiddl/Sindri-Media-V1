...
CREATE PROCEDURE `tactical_userneeds_pageviews_chart_ytd_dbt_10`()
BEGIN
    with cms_data as (
            SELECT siteid, id, date as fdate, Categories, userneeds, tags, tags_r
	...
	agg_next_click_per_article_without_others as(
                SELECT siteid, id, fdate, "Fodsundhed og samfund" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Fodsundhed og samfund.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Forebyggelse" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Forebyggelse.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Forskning og viden" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Forskning og viden.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Praksis" AS tags
                FROM cms_data
	...
	agg_next_click_per_article_without_others_third as(
                SELECT siteid, id, fdate, "Long read" AS tags
                FROM cms_data
                WHERE tags_r REGEXP ".*Long read.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Kort og godt" AS tags
                FROM cms_data
                WHERE tags_r REGEXP ".*Kort og godt.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Q&A" AS tags
                FROM cms_data
                WHERE tags_r REGEXP ".*Q&amp.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Best Practice" AS tags
                FROM cms_data
                WHERE tags_r REGEXP ".*Best Practice.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Viden og forskning" AS tags
                FROM cms_data
                WHERE tags_r REGEXP ".*Viden og forskning.*" 
		),
