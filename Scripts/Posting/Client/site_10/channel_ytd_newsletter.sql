...
CREATE PROCEDURE `Traffic_channel_ytd_newsletter_dbt_10`(
    IN site INT,
...
BEGIN
 ...
 AS Categories,
			...
			WHERE s.siteid = 10 and r.realreferrer = ',channel_label,'
...
''sektion'', coalesce(userneeds, " "),
				   ...
				   ) ), ''[{"id": "10", "date": " ", "clicks": " " , "article": " ", "sektion": " ","BRUGERBEHOV": " ", "brugerbehov": 0}]'' )
                as data
        ...
        visits_per_dayname AS (
			...
			LEFT JOIN prod.traffic_channels tc ON ad.english_day = DAYNAME(tc.`date`)
				...
				AND siteid = 10 and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,10) as siteid,
				ah.series,
