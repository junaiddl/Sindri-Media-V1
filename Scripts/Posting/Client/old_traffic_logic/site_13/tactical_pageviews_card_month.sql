...
CREATE PROCEDURE `tactical_pageviews_card_month_dbt_13`()
BEGIN
    with last_30_article_published as (
        SELECT siteid as siteid,id   FROM prod.site_archive_post
        where 
        date  between DATE_SUB(NOW(), INTERVAL 30 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY) and siteid =13
    ...
    last_30_before_article_published as (
        SELECT siteid as siteid,id   FROM prod.site_archive_post
        where 
        date  between DATE_SUB(NOW(), INTERVAL 60 DAY) and DATE_SUB(NOW(), INTERVAL 30 DAY) and siteid =13
    ...
    agg_last_30_before as (
        ...
        where date  between DATE_SUB(NOW(), INTERVAL 60 DAY) and DATE_SUB(NOW(), INTERVAL 30 DAY) and e.siteid = 13
        group by 1,agg
    ),
    json_data as ( 
