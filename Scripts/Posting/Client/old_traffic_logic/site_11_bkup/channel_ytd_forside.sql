...
CREATE DEFINER=`Site`@`%` PROCEDURE `Traffic_channel_forside_ytd_dbt_11`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
		...
		card_visits_last_year AS (
		 ...
		 final_card as(
			...
			COALESCE(cur_v.siteid, 11) AS siteid,
			...
			FROM (SELECT 11 AS siteid, ''Forside'' AS realreferrer, 0 AS visits) dummy
		...
		channels_table_real AS (
			...
			FROM prod.events
			WHERE
				siteid = 11 and event_action = ''Frontpage'' and
 ...
 AS Categories,
			...
			WHERE s.siteid = 11 and r.realreferrer = ''Forside''
			AND visits >= 5
...
''sektion'', coalesce(Categories, " "),
       ...
       visits_per_dayname AS (
		...
		Result AS (
			SELECT
				COALESCE(lastyear.siteid,11) as siteid,
				ah.series,
