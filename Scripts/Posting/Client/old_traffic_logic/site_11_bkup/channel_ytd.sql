...
CREATE PROCEDURE `Traffic_channel_ytd_dbt_11`(
    IN site INT,
    ...
    SET @sql_query = CONCAT('
        ...
        card_visits_last_month AS (
			...
			LEFT JOIN prod.traffic_channels tc ON
				(rc.realreferrer = tc.realreferrer OR rc.referrer_name = tc.realreferrer)
				AND tc.siteid = 11
				AND tc.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()) - 2, 1)
...
''sektion'', coalesce(Categories, " "),
				   ...
				   ) ), ''[{"id": "11", "date": " ", "clicks": " " , "article": " ", "sektion": " ","BRUGERBEHOV": " ", "brugerbehov": 0}]'' )
                as data
				from site_j
        ),
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id", "label": "ID"}, {"field": "article", "label": "ARTIKEL"}, {"field": "category", "label": "BRUGERBEHOV"}, {"field": "tags", "label": "Tags", "hidden": true}, {"field": "sektion", "label": "Sektion", "hidden": true}, {"field": "date", "label": "PUB DATO"}, {"field": "brugerbehov","label": "Besøg"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
