...
CREATE PROCEDURE `tactical_category_clicks_chart_month_dbt_11`()
BEGIN
    with 
...
AND tags NOT LIKE "%Forskning%")
    ...
    agg_data as (
        ...
        where g.date  between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY)  and g.site_id =  11
    ),
    data_out as(
    select 
        a.siteid as siteid,
