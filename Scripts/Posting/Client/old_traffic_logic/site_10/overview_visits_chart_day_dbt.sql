    ...
    CREATE  PROCEDURE `overview_visits_chart_day_dbt_10`()
    BEGIN
    ...
    WITH agg_on_day as (
        ...
        where date
        between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY) and siteid = 10
        group by 1,2
    ), 
    agg_goal as(
        select site_id,date,sum(Visits_per_day) AS goals from prod.goals
        where  site_id = 10
		AND YEAR(date) = YEAR(CURRENT_DATE())
        group by 1,2
    ),
	agg_prev_year AS (
		SELECT SiteID, date AS date, SUM(Visits) AS pre_year
    ...
    json_data_prev as( 
            SELECT
            COALESCE(ag.site_id, ap.SiteID) as SiteID,"Besøg" as label,"Besøg år til dato grupperet på hhv. dag, uge og måned" as hint ,
            GROUP_CONCAT( CONCAT('"',DATE_FORMAT(ap.date ,  '%d/%m'),'"') order by ap.date  SEPARATOR ', ') AS categories,
            GROUP_CONCAT(COALESCE(goals,0) order by ap.date SEPARATOR ',') as goals,
            GROUP_CONCAT(COALESCE(ap.pre_year, 0) ORDER BY ap.date SEPARATOR ',') AS pre_year
            FROM
                agg_goal ag
    ...
    json_data_curr as( 
            SELECT
            COALESCE(ao.SiteID, ag.site_id) as SiteID,"Besøg" as label,"Besøg år til dato grupperet på hhv. dag, uge og måned" as hint ,
            ...
            where ag.site_id = 10 AND ag.date <= DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
