        ...
        CREATE  PROCEDURE `performance_articles2_card_day_dbt_4`()
        BEGIN
        ...
        left join prod.events e on e.postid=ldp.id 
          and e.siteid = ldp.siteid and e.date between DATE_SUB(NOW(), INTERVAL 8 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY) and e.siteid = 4
        ...
        where l.siteid = 4 -- and l.Event_Action is not null
        union 
        ...
        where p.siteid = 4 -- and -- l.Event_Action is not null
        ),
        ...
        then 1 else 0 end as gt_goal  from last_7days_hits_pages l
