...
CREATE PROCEDURE `tactical_articles_table_month_dbt_4`()
BEGIN
 ...
 AS sektion,
    ...
    cta_per_article as (
        ...
        where Event_Action = 'Newsletter' and  e.siteid = 4
        group by 1,2
    )

    select 
    CONCAT('{"site":',  l.siteid,',"data":{','"columns": [{"field": "id","label": "ID"},{"field": "article","label": "ARTIKEL"},{"field": "sektion","label": "SEKTION"},{"field": "category","label": "BRUGERBEHOV","hidden": true},{"field": "date","label": "DATO"},{"field": "brugerbehov","label": "SIDEVISNINGER"},{"field": "clicks","label": "SIGN-UPS"}],','"rows":',JSON_ARRAYAGG(
            JSON_OBJECT(
                'id', id,
                'article', article,
                'category', l.category,
