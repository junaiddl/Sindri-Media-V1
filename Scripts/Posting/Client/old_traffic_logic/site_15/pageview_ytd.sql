...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        ...
        SET @categories_sql_second = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Leder'',''Tillidsrepræsentant'',''Plejefamilie'',''Senior'',''Privatansat'' ) and siteid = 15'
        );
    ...
    SET @sql_query = CONCAT('
        ...
        pageview_first AS (
            ...
            WHERE siteid = 15 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
                AND r.realreferrer IN ( ''Forside'',''Search'',''NL Hoved'',''NL Tillid'',''NL Leder'',''NL Pleje'' )
        ...
        pivoted_data_second AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''Leder'',''Tillidsrepræsentant'',''Plejefamilie'',''Senior'',''Privatansat'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''Leder'',''Tillidsrepræsentant'',''Plejefamilie'',''Senior'',''Privatansat'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''Leder'',''Tillidsrepræsentant'',''Plejefamilie'',''Senior'',''Privatansat'', "Others")), '']'') AS percentile
			FROM percent_second
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''NL Hoved'',''NL Tillid'',''NL Leder'',''NL Pleje'', "Others")
		),
