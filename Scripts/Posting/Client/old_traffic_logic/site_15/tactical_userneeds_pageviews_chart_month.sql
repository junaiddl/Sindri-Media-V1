...
CREATE PROCEDURE `tactical_userneeds_pageviews_chart_month_dbt_15`()
BEGIN
    with 
	...
	counts_second as (
		...
		categories_d_second as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Leder','Presse','Privatansat','Plejefamilie','Tillidsrepræsentant','Lærere','Arbejdsmiljørepræsentant', 'TRIO', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Leder','Presse','Privatansat','Plejefamilie','Tillidsrepræsentant','Lærere','Arbejdsmiljørepræsentant', 'TRIO', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Leder','Presse','Privatansat','Plejefamilie','Tillidsrepræsentant','Lærere','Arbejdsmiljørepræsentant', 'TRIO', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Leder','Presse','Privatansat','Plejefamilie','Tillidsrepræsentant','Lærere','Arbejdsmiljørepræsentant', 'TRIO', 'others' ) SEPARATOR ',') AS top_10
