...
CREATE PROCEDURE `Traffic_channel_week_dbt_15`(
    IN site INT,
    ...
    SET @sql_query = CONCAT('
        ...
        card_visits AS (
			...
			LEFT JOIN prod.traffic_channels tc ON
				...
				AND tc.siteid = 15
				AND tc.date BETWEEN DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY)
			GROUP BY 1,2
        ),
        card_visits_last_month AS (
			...
			LEFT JOIN prod.traffic_channels tc ON
				(rc.realreferrer = tc.realreferrer OR rc.referrer_name = tc.realreferrer)
				AND tc.siteid = 15
				AND tc.date BETWEEN DATE_SUB(NOW(), INTERVAL 15 DAY) AND DATE_SUB(NOW(), INTERVAL 8 DAY)
			GROUP BY 1,2
         ),
         final_card as(
           SELECT
...
''sektion'', coalesce(Categories, " "),
				   ...
				   ) ), ''[{"id": "15", "date": " ", "clicks": " " , "article": " ", "sektion": " ","BRUGERBEHOV": " ", "brugerbehov": 0}]'' )
                as data
				from site_j
        ),
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id","label": "ID"},{"field": "article","label": "ARTIKEL"},{"field": "category","label": "BRUGERBEHOV"},{"field": "sektion","label": "MÅLGRUPPER", "hidden" : true},{"field": "tags","label": "Tags", "hidden" : true},{"field": "emner","label": "Emner", "hidden" : true},{"field": "date","label": "DATO"},{"field": "brugerbehov","label": "Besøg"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
