...
CREATE DEFINER=`Site`@`%` PROCEDURE `Traffic_channel_forside_month_dbt_15`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
		...
		card_visits_last_year AS (
			...
			FROM prod.events
			where siteid = 15 and event_action = ''Frontpage''
			AND date BETWEEN DATE_SUB(NOW(), INTERVAL 61 DAY) AND DATE_SUB(NOW(), INTERVAL 31 DAY)
			GROUP BY 1,2
		 ),
		 final_card as(
			SELECT
			...
			COALESCE(cur_v.siteid, 15) AS siteid,
			...
			FROM (SELECT 15 AS siteid, ''Forside'' AS realreferrer, 0 AS visits) dummy
		...
		channels_table_real AS (
			...
			FROM prod.events
			WHERE
				siteid = 15 and event_action = ''Frontpage'' and
 ...
 AS Categories,
			...
			WHERE s.siteid = 15 and r.realreferrer = ''Forside''
...
''sektion'', coalesce(Categories, " "),
       ...
       visits_per_dayname AS (
		...
		Result AS (
			SELECT
				COALESCE(lastyear.siteid,15) as siteid,
				ah.series,
