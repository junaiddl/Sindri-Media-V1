...
CREATE PROCEDURE `Traffic_channel_ytd_dbt_15`(
    IN site INT,
    ...
    SET @sql_query = CONCAT('
        ...
        card_visits AS (
			...
			LEFT JOIN prod.traffic_channels tc ON
				...
				AND tc.siteid = 15
				AND tc.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1)
        ...
        card_visits_last_month AS (
			...
			LEFT JOIN prod.traffic_channels tc ON
				(rc.realreferrer = tc.realreferrer OR rc.referrer_name = tc.realreferrer)
				AND tc.siteid = 15
				AND tc.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()) - 1, 1)
