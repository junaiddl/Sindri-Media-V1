...
CREATE PROCEDURE `tactical_category_pageviews_chart_month_dbt_16`()
BEGIN
		with cms_data as (
            ...
            last_30_days_article_published_without_others_second as(
                SELECT siteid, id, fdate, "Løn og konkrete fordele" AS tags
                FROM cms_data
                ...
                SELECT siteid, id, fdate, "Det moderne arbejdsliv" AS tags
                FROM cms_data
                ...
                SELECT siteid, id, fdate, "Grøn omstilling" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*Grøn_omstilling.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Formanden har ordet" AS tags
                FROM cms_data
