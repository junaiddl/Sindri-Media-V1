...
CREATE PROCEDURE `tactical_userneeds_pageviews_chart_month_dbt_24`()
BEGIN
	with cms_data as (
	 ...
	 last_30_days_article_published_Agg as(
      ...
      (
		...
		left join prod.events_string a on a.postid=e.id and a.siteid = e.siteid and a.event_action = 'Next Click'
		where    e.siteid = 24
		group by 1,2,3
		),
	...
	counts as (
		...
		last_30_days_article_published_without_others_second as(
                SELECT siteid, id, fdate, "BUPL.dk" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL.dk.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Hovedstaden" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Hovedstaden.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Sydjylland" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Sydjylland.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Nordsjælland" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Nordsjælland.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Sydøst" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Sydøst.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Storkøbenhavn" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Storkøbenhavn.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Fyn" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Fyn.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Østjylland" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Østjylland.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Århus" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Århus.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Midtsjælland" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Midtsjælland.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL MidtVestjylland" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL MidtVestjylland.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Nordjylland" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*BUPL Nordjylland.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "BUPL Bornholm" AS tags
                FROM cms_data
	...
	counts_second as (
		...
		categories_d_second as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'BUPL.dk','BUPL Hovedstaden','BUPL Sydjylland','BUPL Nordsjælland','BUPL Sydøst','BUPL Storkøbenhavn','BUPL Fyn','BUPL Østjylland','BUPL Århus','BUPL Midtsjælland','BUPL MidtVestjælland','BUPL Nordjylland','BUPL Bornholm', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'BUPL.dk','BUPL Hovedstaden','BUPL Sydjylland','BUPL Nordsjælland','BUPL Sydøst','BUPL Storkøbenhavn','BUPL Fyn','BUPL Østjylland','BUPL Århus','BUPL Midtsjælland','BUPL MidtVestjælland','BUPL Nordjylland','BUPL Bornholm', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'BUPL.dk','BUPL Hovedstaden','BUPL Sydjylland','BUPL Nordsjælland','BUPL Sydøst','BUPL Storkøbenhavn','BUPL Fyn','BUPL Østjylland','BUPL Århus','BUPL Midtsjælland','BUPL MidtVestjælland','BUPL Nordjylland','BUPL Bornholm', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'BUPL.dk','BUPL Hovedstaden','BUPL Sydjylland','BUPL Nordsjælland','BUPL Sydøst','BUPL Storkøbenhavn','BUPL Fyn','BUPL Østjylland','BUPL Århus','BUPL Midtsjælland','BUPL MidtVestjælland','BUPL Nordjylland','BUPL Bornholm', 'others' ) SEPARATOR ',') AS top_10
