...
CREATE PROCEDURE `Traffic_dynamic_linequery_day_dbt_24`(
	IN site INT,
...
BEGIN
   SET SESSION group_concat_max_len = 100000;
    ...
    SET @referrers_sql = CONCAT(
        'SELECT DISTINCT realreferrer FROM pre_stage.ref_value WHERE realreferrer IN ( ''Direct'',''Search'',''Facebook'', ''Newsletter''  ) and siteid = 24'
    );
    ...
    SET @sql_query = CONCAT('
		...
		agg_on_day AS (
			...
			LEFT JOIN prod.traffic_channels_string t ON d.date = t.date AND t.siteid = 24
