...
CREATE PROCEDURE `tactical_pageviews_card_month_dbt_24`()
BEGIN
    with last_30_article_published as (
        SELECT siteid as siteid,id   FROM prod.site_archive_post_string
        where 
        date  between DATE_SUB(NOW(), INTERVAL 30 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY) and siteid =24
    ...
    last_30_article_published_Agg as(
        ...
        agg_last_30 as (
        select  e.siteid as siteid,sum(e.unique_pageviews)/agg as value from prod.pages_string e
    ...
    last_30_before_article_published as (
        SELECT siteid as siteid,id   FROM prod.site_archive_post_string
        where 
        date  between DATE_SUB(NOW(), INTERVAL 60 DAY) and DATE_SUB(NOW(), INTERVAL 30 DAY) and siteid =24
    ...
    agg_last_30_before as (
        ...
        where date  between DATE_SUB(NOW(), INTERVAL 60 DAY) and DATE_SUB(NOW(), INTERVAL 30 DAY) and e.siteid = 24
        group by 1,agg
    ),
    json_data as ( 
