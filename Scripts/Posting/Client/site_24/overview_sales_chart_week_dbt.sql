        ...
        CREATE PROCEDURE `overview_sales_chart_week_dbt_24`()
        BEGIN
		...
		json_data_curr AS (
            ...
            WHERE
                ag.site_id = 24 and ag.week <= CAST(WEEK(DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY), 3) as char)
