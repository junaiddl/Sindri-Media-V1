...
CREATE PROCEDURE `Traffic_channel_week_newsletter_dbt_24`(
    IN site INT,
...
BEGIN
 ...
 AS Categories,
			...
			WHERE s.siteid = 24 and r.realreferrer = ',channel_label,'
...
''sektion'', coalesce(Categories, " "),
		...
		WeekVisits_chartbar AS (
			...
			LEFT JOIN prod.traffic_channels_string tc ON ad.english_day = DAYNAME(tc.`date`)
				...
				AND siteid = 24 and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,24) as siteid,
				ah.series,
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id","label": "ID"},{"field": "article","label": "ARTIKEL"},{"field": "category","label": "BRUGERBEHOV"},{"field": "sektion", "label": "AFDELING", "hidden": true},{"field": "tags", "label": "tags", "hidden": true},{"field": "date","label": "DATO"},{"field": "brugerbehov","label": "Kliks"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
