...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_24`(
    IN site INT,
...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        ...
        WHERE realreferrer IN ( ''Forside'',''Search'',''Facebook'',''Newsletter'' ) and siteid = 24'
    );
    
        SET @categories_sql_first = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Opdater mig'',''Forbind mig'',''Hjælp mig med at forstå'',''Giv mig en fordel'',''Underhold mig'',''Inspirer mig'' ) and siteid = 24'
        );
    
        SET @categories_sql_second = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''BUPL.dk'',''BUPL Hovedstaden'',''BUPL Sydjylland'',''BUPL Nordsjælland'',''BUPL Sydøst'',''BUPL Storkøbenhavn'',''BUPL Fyn'',''BUPL Østjælland'',''BUPL Århus'',''BUPL Midtsjælland'',''BUPL MidtVestjælland'',''BUPL Nordjylland'',''BUPL Bornholm'' ) and siteid = 24'
        );
    ...
    SET @sql_query = CONCAT('
        ...
        combinations_first AS (
            ...
            ORDER BY FIELD(r.realreferrer, ''Forside'',''Search'',''Facebook'',''Newsletter'', "Others")
        ),
        pageview_first AS (
            SELECT 
                siteid,
            ...
            WHERE siteid = 24 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
        ...
        main_first AS (
            ...
            WHERE siteid = 24 AND Event_Action = ''Frontpage'' 
        ...
        transformed_data_first AS (
            SELECT
                ...
                t_totals,
                CASE 
                    WHEN event_name LIKE ''://%'' THEN CONCAT(''https://bupl.dk/'', SUBSTRING(event_name, 5))
                    ELSE event_name
                END AS modifyurl
            FROM main_first
        ),
        ...
        summed_data_first AS (
			...
			ORDER BY FIELD(c.realreferrer, ''Forside'',''Search'',''Facebook'',''Newsletter'', "Others")
        ),
        percent_first AS (
            SELECT
                realreferrer,
        ...
        pivoted_data_first AS (
			...
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''Facebook'',''Newsletter'', "Others")
		),
        final_first as(
			SELECT 	
        ...
        cms_groups_without_others_second as(
            SELECT siteid, id, date, "BUPL.dk" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL.dk.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Hovedstaden" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Hovedstaden.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Sydjylland" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Sydjylland.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Nordsjælland" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Nordsjælland.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Sydøst" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Sydøst.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Storkøbenhavn" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Storkøbenhavn.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Fyn" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Fyn.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Østjylland" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Østjylland.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Århus" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Århus.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Midtsjælland" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Midtsjælland.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL MidtVestjylland" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL MidtVestjylland.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Nordjylland" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Nordjylland.*"
            UNION ALL
            
            SELECT siteid, id, date, "BUPL Bornholm" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*BUPL Bornholm.*"
        ),
        ...
        pivoted_data_second AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''BUPL.dk'',''BUPL Hovedstaden'',''BUPL Sydjylland'',''BUPL Nordsjælland'',''BUPL Sydøst'',''BUPL Storkøbenhavn'',''BUPL Fyn'',''BUPL Østjælland'',''BUPL Århus'',''BUPL Midtsjælland'',''BUPL MidtVestjælland'',''BUPL Nordjylland'',''BUPL Bornholm'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''BUPL.dk'',''BUPL Hovedstaden'',''BUPL Sydjylland'',''BUPL Nordsjælland'',''BUPL Sydøst'',''BUPL Storkøbenhavn'',''BUPL Fyn'',''BUPL Østjælland'',''BUPL Århus'',''BUPL Midtsjælland'',''BUPL MidtVestjælland'',''BUPL Nordjylland'',''BUPL Bornholm'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''BUPL.dk'',''BUPL Hovedstaden'',''BUPL Sydjylland'',''BUPL Nordsjælland'',''BUPL Sydøst'',''BUPL Storkøbenhavn'',''BUPL Fyn'',''BUPL Østjælland'',''BUPL Århus'',''BUPL Midtsjælland'',''BUPL MidtVestjælland'',''BUPL Nordjylland'',''BUPL Bornholm'', "Others")), '']'') AS percentile
			FROM percent_second
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''Facebook'',''Newsletter'', "Others")
		),
