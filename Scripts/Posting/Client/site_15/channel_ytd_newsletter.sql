...
CREATE PROCEDURE `Traffic_channel_ytd_newsletter_dbt_15`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        card_visits AS (
            SELECT
				...
				COALESCE(SUM(n.visits), 0) AS visits
			FROM referrer rc
			LEFT JOIN prod.newsletter n ON
				rc.realreferrer = n.referrer_name
				AND n.site_id = 15
				AND n.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1)
        ...
        card_visits_last_month AS (
            SELECT
				n.site_id as siteid,
				n.referrer_name as realreferrer,
				COALESCE(SUM(n.visits), 0) AS last_visits
			FROM referrer rc
			LEFT JOIN prod.newsletter n ON
				rc.realreferrer = n.referrer_name
				AND n.site_id = 15
				AND n.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()) - 1, 1)
		...
		channels_table_real AS (
			SELECT
				...
				post_id AS postid,
				Date,
				referrer_name as realreferrer,
				SUM(visits) AS visits
			FROM prod.newsletter n
			WHERE
				site_id = 15 and
				referrer_name = ',channel_label,' and post_id is not  null and
 ...
 AS Categories,
                        ...
                        CASE
                            WHEN s.Tags REGEXP ".*Børn og unge|Anbringelse|Sikrede institutioner.*" THEN "Børn og unge"
                            WHEN s.Tags REGEXP ".*Socialpolitik|Nyheder|Forbund og a-kasse|Coronavirus.*" THEN "Andet"
                            WHEN s.Tags REGEXP ".*Autisme|Udviklingshandicap|Botilbud|Magtanvendelse|Seksualitet|Domfældte.*" THEN "Handicap"
                            WHEN s.Tags REGEXP ".*Psykiatri|Misbrug|Væresteder|Hjemløse|Herberg og forsorgshjem|Marginaliserede.*" THEN "Psykiatri og udsathed"
                            WHEN s.Tags REGEXP ".*Socialpædagogisk faglighed|Socialpædagogisk praksis|Metoder og tilgange|Etik.*" THEN "Socialpædagogisk faglighed"
                            WHEN s.Tags REGEXP ".*Ansættelsesvilkår|Opsigelse|Overenskomst|Arbejdstid|Løn|Ferie|Barsel|Senior |Ligestilling|Job og karriere|Uddannelse|Meritpædagoguddannelse|Kompetenceudvikling |Efteruddannelse|Arbejdsmiljø|Stress|Sygdom|Vold og trusler|Arbejdsskade|PTSD|A-kasse |Lønforsikring|Ledighed|Efterløn|Dagpenge|Arbejdsløshed.*" THEN "Arbejdsliv og vilkår"
                        END AS Emner,
			...
			WHERE s.siteid = 15 and r.realreferrer = ',channel_label,'
...
''sektion'', coalesce(Categories, " "),
        ...
        visits_per_dayname AS (
			...
			LEFT JOIN prod.traffic_channels tc ON ad.english_day = DAYNAME(tc.`date`)
				AND date BETWEEN  DATE_SUB(CURDATE(), INTERVAL 1 YEAR) AND DATE_SUB(CURDATE(), INTERVAL 1 DAY)
				AND siteid = 15 and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		...
		LastWeekVisits AS (
			...
			WHERE date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()) - 1, 1)
                            AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
				AND siteid = 15
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,15) as siteid,
				ah.series,
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id","label": "ID"},{"field": "article","label": "ARTIKEL"},{"field": "category","label": "BRUGERBEHOV"},{"field": "sektion","label": "MÅLGRUPPER", "hidden" : true},{"field": "tags","label": "Tags", "hidden" : true},{"field": "emner","label": "Emner", "hidden" : true},{"field": "date","label": "DATO"},{"field": "brugerbehov","label": "Kliks"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
