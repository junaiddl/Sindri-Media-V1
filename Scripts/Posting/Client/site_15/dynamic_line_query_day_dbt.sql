...
CREATE PROCEDURE `Traffic_dynamic_linequery_day_dbt_15`(
	IN site INT,
...
BEGIN
   SET SESSION group_concat_max_len = 100000;
    ...
    SET @referrers_sql = CONCAT(
        'SELECT DISTINCT realreferrer FROM pre_stage.ref_value WHERE realreferrer IN ( ''Direct'',''Search'',''Newsletter'',''NL Hoved'', ''NL Tillid'',''NL Leder'', ''NL Pleje'' ) and siteid = 15'
    );
    ...
    SET @sql_query = CONCAT('
		...
		agg_on_day AS (
			...
			LEFT JOIN prod.traffic_channels t ON d.date = t.date AND t.siteid = 15
		...
		json_data as (
			SELECT
				realreferrer,
				GROUP_CONCAT(CONCAT(''"'', DATE_FORMAT(date, ''%Y/%m/%d''), ''"'') ORDER BY date SEPARATOR '', '') AS categories,
		...
		final_ar as (
			...
			''Besøg'' as label, ''Besøg opdelt på trafikkanaler'' as hint,
			  categories AS categories,
				GROUP_CONCAT(realreferrer ORDER BY FIELD(LOWER(realreferrer),  ''Direct'',''Search'', ''Newsletter'' , ''others'' ) SEPARATOR '','') AS series,
			  GROUP_CONCAT(referrer_counts ORDER BY FIELD(LOWER(realreferrer),  ''Direct'',''Search'', ''Newsletter'' , ''others'') SEPARATOR '','') AS data
			FROM json_data
			GROUP BY categories
		)
        SELECT
