...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_15`(
    IN site INT,
...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        ...
        WHERE realreferrer IN ( ''Forside'',''Search'',''NL Hoved'',''NL Tillid'',''NL Leder'',''NL Pleje'' ) and siteid = 15'
    );
    
        SET @categories_sql_first = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Opdater mig'',''Forbind mig'',''Hjælp mig med at forstå'',''Giv mig en fordel'',''Underhold mig'',''Inspirer mig'' ) and siteid = 15'
        );
    
        SET @categories_sql_second = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Presse'',''Privatansat'',''Lærere'', ''TRIO'', ''Senior'', ''Nyuddannet'', ''Studerende'' ) and siteid = 15'
        );
    
        SET @categories_sql_third = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Arbejdsliv og vilkår'', ''Socialpædagogisk faglighed'',''Handicap'',''Børn og unge'',''Psykiatri og sundhed'',''Andet'' ) and siteid = 15'
        );
    ...
    SET @sql_query = CONCAT('
        ...
        combinations_first AS (
            ...
            ORDER BY FIELD(r.realreferrer, ''Forside'',''Search'',''NL Hoved'',''NL Tillid'',''NL Leder'',''NL Pleje'', "Others")
        ),
        pageview_first AS (
            SELECT 
                siteid,
            ...
            WHERE siteid = 15 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
                AND r.realreferrer IN ( ''Forside'', ''Search'' )
        ...
        main_first AS (
            ...
            FROM prod.events
            WHERE siteid = 15 AND Event_Action = ''Frontpage'' 
        ...
        transformed_data_first AS (
            SELECT
                ...
                t_totals,
                CASE 
                    WHEN event_name LIKE ''://%'' THEN CONCAT(''https://sl.dk/'', SUBSTRING(event_name, 5))
                    ELSE event_name
                END AS modifyurl
            FROM main_first
        ),
        ...
        newsletter_data_first AS (
            SELECT 
                ...
                AND n.referrer_name IN ( ''NL Hoved'',''NL Tillid'',''NL Leder'',''NL Pleje'' )
        ...
        summed_data_first AS (
			...
			ORDER BY FIELD(c.realreferrer, ''Forside'',''Search'',''NL Hoved'',''NL Tillid'',''NL Leder'',''NL Pleje'', "Others")
        ),
        percent_first AS (
            SELECT
                realreferrer,
        ...
        pivoted_data_first AS (
			...
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''NL Hoved'',''NL Tillid'',''NL Leder'',''NL Pleje'', "Others")
		),
        final_first as(
			SELECT 	
        ...
        cms_groups_without_others_second as(
            SELECT siteid, id, date, "Presse" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Presse.*"
            UNION ALL
            
            SELECT siteid, id, date, "Privatansat" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Privatansat.*"
            UNION ALL
            
            SELECT siteid, id, date, "Lærere" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Lærere.*"
            UNION ALL
            
            SELECT siteid, id, date, "TRIO" AS tags, link
            FROM cms_data
            ...
            SELECT siteid, id, date, "Nyuddannet" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Nyuddannet.*"
            UNION ALL
            
            SELECT siteid, id, date, "Studerende" AS tags, link
            FROM cms_data
        ...
        pivoted_data_second AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''Presse'',''Privatansat'',''Lærere'', ''TRIO'', ''Senior'', ''Nyuddannet'', ''Studerende'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''Presse'',''Privatansat'',''Lærere'', ''TRIO'', ''Senior'', ''Nyuddannet'', ''Studerende'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''Presse'',''Privatansat'',''Lærere'', ''TRIO'', ''Senior'', ''Nyuddannet'', ''Studerende'', "Others")), '']'') AS percentile
			FROM percent_second
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''NL Hoved'',''NL Tillid'',''NL Leder'',''NL Pleje'', "Others")
		),
        ...
        cms_groups_without_others_third as(
            SELECT siteid, id, date, "Børn og unge" AS tags, link
            FROM cms_data
        ...
        pivoted_data_third AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''Arbejdsliv og vilkår'', ''Socialpædagogisk faglighed'',''Handicap'',''Børn og unge'',''Psykiatri og sundhed'',''Andet'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''Arbejdsliv og vilkår'', ''Socialpædagogisk faglighed'',''Handicap'',''Børn og unge'',''Psykiatri og sundhed'',''Andet'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''Arbejdsliv og vilkår'', ''Socialpædagogisk faglighed'',''Handicap'',''Børn og unge'',''Psykiatri og sundhed'',''Andet'', "Others")), '']'') AS percentile
			FROM percent_third
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''NL Hoved'',''NL Tillid'',''NL Leder'',''NL Pleje'', "Others")
		),
