...
CREATE PROCEDURE `Traffic_channel_week_newsletter_dbt_15`(
    IN site INT,
    ...
    SET @sql_query = CONCAT('
        ...
        card_visits_last_month AS (
			...
			LEFT JOIN prod.newsletter n ON
				rc.realreferrer = n.referrer_name
				AND n.site_id = 15
				AND n.date BETWEEN DATE_SUB(NOW(), INTERVAL 15 DAY) AND DATE_SUB(NOW(), INTERVAL 8 DAY)
			GROUP BY 1,2
         ),
         final_card as(
           SELECT
...
''sektion'', coalesce(Categories, " "),
		...
		TotalWeekVisits_chartbar AS (
			SELECT
				SUM(visits) as total_visits
			FROM WeekVisits_chartbar
		),
		percentile_chartbar AS (
			SELECT
				categories,
				visits,
				COALESCE((ROUND((visits / (SELECT total_visits FROM TotalWeekVisits_chartbar) * 100), 2)),0) AS series
			FROM WeekVisits_chartbar
		),
		final_chartbar AS (
			SELECT
