  ...
  (
      ...
      AND tags REGEXP "(^|[ ,])(dansk|it|matematik|specialpædagogik|Skolepolitik|DLF|Skoleledelse|Psykisk arbejdsmiljø|Forskning)([,]|$)"
            ...
            last_30_days_article_published_without_others_second as(
                ...
                SELECT siteid, id, fdate, "inspiration" AS tags
                FROM cms_data
            ...
            last_30_days_article_published_without_others_third as(
                SELECT siteid, id, fdate, "skolepolitik" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*skolepolitik.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "DLF" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*DLF.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "skoleledelse" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*skoleledelse.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "psykisk arbejdsmiljø" AS tags
                FROM cms_data
                WHERE Tags REGEXP ".*psykisk arbejdsmiljø.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "forskning" AS tags
                FROM cms_data
            ...
            last_30_days_article_published_without_others_fourth as(
            ...
            last_30_days_article_published_fourth as (
                SELECT a.siteid, a.id, a.fdate, 'others' as Tags
                FROM cms_data a
                LEFT JOIN last_30_days_article_published_without_others_fourth b
                ON a.siteid = b.siteid AND a.id = b.id
                WHERE b.siteid IS NULL

                UNION ALL

                SELECT * FROM last_30_days_article_published_without_others_fourth
            ),
            cta_per_article_fourth as (
                select  e.siteid as siteid,e.postid,a.tags,sum(unique_pageviews) as val from last_30_days_article_published_fourth a
                left join  prod.pages e on a.id=e.postid
                and e.date between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY) 
                group by 1,2,3
            ),
            agg_cta_per_article_fourth as(
                select siteid as siteid,tags, coalesce(sum(val), 0) as agg_sum from  cta_per_article_fourth
                where siteid = 11
                group by 1,2
            ),
            last_30_days_article_published_Agg_fourth as(
                select siteid as siteid,tags,count(*) as agg from  cta_per_article_fourth
                where siteid = 11
                group by 1,2
            ),
