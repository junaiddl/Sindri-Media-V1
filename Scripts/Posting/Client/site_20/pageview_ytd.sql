...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_20`(
    IN site INT,
...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        ...
        WHERE realreferrer IN ( ''Forside'',''Search'',''Facebook'',''Newsletter'' ) and siteid = 20'
    );
    
        SET @categories_sql_first = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Opdater mig'',''Forbind mig'',''Hjælp mig med at forstå'',''Giv mig en fordel'',''Underhold mig'',''Inspirer mig'' ) and siteid = 20'
        );
    ...
    SET @sql_query = CONCAT('
        ...
        pageview_first AS (
            ...
            WHERE siteid = 20 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
        ...
        main_first AS (
            ...
            FROM prod.events
            WHERE siteid = 20 AND Event_Action = ''Frontpage'' 
        ...
        transformed_data_first AS (
            SELECT
                ...
                t_totals,
                CASE 
                    WHEN event_name LIKE ''://%'' THEN CONCAT(''https://fagbladetboligen.dk/'', SUBSTRING(event_name, 5))
                    ELSE event_name
                END AS modifyurl
            FROM main_first
        ),
