		...
		CREATE PROCEDURE `performance_articles1_card_month_dbt_20`()
		BEGIN
		...
		last_30days_hits as(
			...
			and e.date between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY) and e.siteid = 20
		...
		last_30days_before_hits as(
			...
			left join prod.events e
			on e.postid=ldp.id and ldp.siteid=e.siteid
			and e.date between DATE_SUB(NOW(), INTERVAL 61 DAY) and DATE_SUB(NOW(), INTERVAL  31 DAY) and e.siteid = 20
			and e.Event_Action = "Next Click"
			group by 1,2,3
		),
		...
		last_30days_pageview_before as(
			...
			join prod.pages e
			on e.postid=ldp.id and e.siteid = ldp.siteid
			where e.date between DATE_SUB(NOW(), INTERVAL 61 DAY) and DATE_SUB(NOW(), INTERVAL  31 DAY) and e.siteid = 20
			group by 1,2,3
		),
