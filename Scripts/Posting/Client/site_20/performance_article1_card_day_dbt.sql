		...
		CREATE  PROCEDURE `performance_articles1_card_day_dbt_20`()
		BEGIN
		...
		last_7days_before_hits as(
			...
			join prod.events e
			on e.postid=ldp.id
			and e.date between DATE_SUB(NOW(), INTERVAL 15 DAY) and DATE_SUB(NOW(), INTERVAL  8 DAY)  and ldp.siteid = 20
			and e.Event_Action = 'Next Click'
			group by 1,2,3
		),
		last_7days_pageview as(
			...
			join prod.pages e
			on e.postid=ldp.id and ldp.siteid=e.siteid
			and e.date between DATE_SUB(NOW(), INTERVAL 8 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY)  and ldp.siteid = 20
			group by 1,2,3
		),
