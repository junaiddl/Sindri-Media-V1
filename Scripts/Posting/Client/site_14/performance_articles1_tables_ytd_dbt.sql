        ...
        CREATE DEFINER=`Site`@`%` PROCEDURE `performance_articles1_tables_ytd_dbt_14`()
        BEGIN
 ...
 AS sektion,
        ...
        last_ytd_hits as (
            ...
            FROM   
                last_ytd_post ldp
                LEFT JOIN prod.events e ON e.postid = ldp.id AND e.siteid = ldp.siteid AND e.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY) AND e.siteid = 14 AND e.Event_Action =  'Next Click'
        ...
        last_ytd_pageview as (
            ...
            FROM 
                last_ytd_post ldp
                LEFT JOIN prod.pages e ON e.postid = ldp.id AND e.siteid = ldp.siteid AND e.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY) AND e.siteid = 14
            GROUP BY 1, 2, 3, 5, 7, 8, 9,10
        ...
        select 
        CONCAT('{"site":', siteid,',"data":{','"columns":[ {"field": "id", "label": "ID"}, {"field": "article", "label": "ARTIKEL"}, {"field": "category", "label": "BRUGERBEHOV"}, {"field": "sektion", "label": "ARTIKELTYPE", "hidden": true}, {"field": "tags", "label": "EMNER", "hidden": true}, {"field": "date", "label": "DATO"}, {"field": "brugerbehov", "label": "SIDEVISNINGER"}, {"field": "clicks", "label": "NEXT CLICK (%)"} ],','"rows":',JSON_ARRAYAGG(
                JSON_OBJECT(
                    'id', postid,
                    'article', Title,
        ...
        from last_ytd_hits_pages_goal lg where gt_goal  =1 and siteid = 14;
