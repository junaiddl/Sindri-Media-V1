...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_14`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        pageview_first AS (
            ...
            WHERE siteid = 14 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
        ...
        newsletter_data_first AS (
            SELECT 
                ...
                AND n.referrer_name IN ( ''NL <47'', ''NL 48-60'', ''NL >60'' )
        ...
        cms_groups_without_others_second as(
            ...
            SELECT siteid, id, date, "om folkeskolen" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*om folkeskolen.*"
            UNION ALL
            
            SELECT siteid, id, date, "folkeskolen" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*folkeskolen.*"
            UNION ALL
            
            SELECT siteid, id, date, "nyheder" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*nyheder.*"
            UNION ALL
            
            SELECT siteid, id, date, "kalender" AS tags, link
            FROM cms_data
            ...
            SELECT siteid, id, date, "kommunalvalg 2021" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*kommunalvalg 2021.*"
            UNION ALL
            
            SELECT siteid, id, date, "biologi" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*biologi.*"
            UNION ALL
            
            SELECT siteid, id, date, "lærerliv" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*lærerliv.*"
            UNION ALL
            
            SELECT siteid, id, date, "søndervangskolen" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*søndervangskolen.*"
            UNION ALL
            
            SELECT siteid, id, date, "trivsel" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*trivsel.*"
            UNION ALL
            
            SELECT siteid, id, date, "corona" AS tags, link
            FROM cms_data
            ...
            SELECT siteid, id, date, "stu" AS tags, link
            FROM cms_data
            ...
            SELECT siteid, id, date, "skolelederforeningen" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*skolelederforeningen.*"
            UNION ALL
            
            SELECT siteid, id, date, "nationale test" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*nationale test.*"
            UNION ALL
            
            SELECT siteid, id, date, "læreruddannelsen" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*læreruddannelsen.*"
            UNION ALL
            
            SELECT siteid, id, date, "elevplaner" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*elevplaner.*"
            UNION ALL
            
            SELECT siteid, id, date, "autisme-spektret" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*autisme-spektret.*"
            UNION ALL
            
            SELECT siteid, id, date, "lærermangel" AS tags, link
            FROM cms_data
            ...
            SELECT siteid, id, date, "anmeldelser" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*anmeldelser.*"
            UNION ALL
            
            SELECT siteid, id, date, "sponseret indhold" AS tags, link
            FROM cms_data
        ...
        cms_groups_without_others_third as(
            ...
            SELECT siteid, id, date, "Social dumping" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Social dumping.*"
            UNION ALL
            
            SELECT siteid, id, date, "Ulighed" AS tags, link
            FROM cms_data
            ...
            SELECT siteid, id, date, "Fagligt" AS tags, link
            FROM cms_data
