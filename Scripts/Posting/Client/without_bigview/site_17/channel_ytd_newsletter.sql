...
CREATE PROCEDURE `Traffic_channel_ytd_newsletter_dbt_17`(
    IN site INT,
...
BEGIN
 ...
 AS Categories,
			...
			WHERE s.siteid = 17 and r.realreferrer = ',channel_label,'
...
''sektion'', coalesce(Categories, " "),
        ...
        visits_per_dayname AS (
			...
			LEFT JOIN prod.traffic_channels tc ON ad.english_day = DAYNAME(tc.`date`)
				...
				AND siteid = 17 and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,17) as siteid,
				ah.series,
