...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_17`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        pageview_first AS (
            ...
            WHERE siteid = 17 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
        ...
        newsletter_data_first AS (
            SELECT 
                ...
                AND n.referrer_name IN ( ''NL H'', ''NL S'', ''NL F'',''NL B'', ''NL C'' )
        ...
        grouped_visits_first AS (
            ...
            LEFT JOIN prod.site_archive_post s
