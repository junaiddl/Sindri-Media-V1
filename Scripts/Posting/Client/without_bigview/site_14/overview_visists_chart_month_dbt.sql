      ...
      CREATE  PROCEDURE `overview_visits_chart_month_dbt_14`()
      BEGIN
      ...
      WITH agg_on_month as (
        ...
        where date
        between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY) and siteid= 14
        group by 1,2,3
      ),
      agg_goal as(
        select site_id,month(date) as month,DATE_FORMAT(date,'%b') as monthname,sum(Visits_per_day) AS goals from prod.goals
          where site_id =  14 and YEAR(date) = YEAR(CURRENT_DATE())
        group by 1,2,3
      ),
      agg_prev_year AS (
