...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        cms_groups_without_others_second as(
            SELECT siteid, id, date, "Nyhedsoverblik" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Nyhedsoverblik.*"
            UNION ALL
            ...
            SELECT siteid, id, date, "Digidoc" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Digidoc.*"
            UNION ALL
            
            SELECT siteid, id, date, "post" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*post.*"
            UNION ALL
            
            SELECT siteid, id, date, "news_overview" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*news_overview.*"
        ),
