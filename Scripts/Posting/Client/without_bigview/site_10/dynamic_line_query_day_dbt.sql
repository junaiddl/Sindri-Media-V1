...
CREATE PROCEDURE `Traffic_dynamic_linequery_day_dbt_10`(
	IN site INT,
...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        'SELECT DISTINCT realreferrer FROM pre_stage.ref_value WHERE realreferrer IN ( ''Direct'', ''Search'', ''Facebook'', ''Newsletter'' ) and siteid = 10'
    );
    ...
    SET @sql_query = CONCAT('
		...
		agg_on_day AS (
			...
			LEFT JOIN prod.traffic_channels t ON d.date = t.date AND t.siteid = 10
