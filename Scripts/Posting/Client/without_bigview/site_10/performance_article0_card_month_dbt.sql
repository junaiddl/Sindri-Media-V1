			...
			CREATE PROCEDURE `performance_articles0_card_month_dbt_10`()
			BEGIN
			...
			and e.date between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY) and e.siteid = 10
			...
			last_30days_before_hits as(
				...
				join prod.events e
				on e.postid=ldp.id and e.siteid = ldp.siteid
				and e.date between DATE_SUB(NOW(), INTERVAL 61 DAY) and DATE_SUB(NOW(), INTERVAL  31 DAY) and e.siteid = 10
				and e.Event_Action = 'Newsletter'
				group by 1,2,3,5
			),
			...
			join prod.pages e
			on e.postid=ldp.id and e.siteid = ldp.siteid
			where e.date between DATE_SUB(NOW(), INTERVAL 61 DAY) and DATE_SUB(NOW(), INTERVAL  31 DAY) and e.siteid = 10
			group by 1,2,3
			),
			...
			case when (coalesce(l.hits,0)<coalesce(g.Min_CTA,0) and coalesce(l.pageviews,0)<coalesce(g.Min_pageviews,0) ) then 1 else 0 end as gt_goal  from last_30days_hits_pages l
