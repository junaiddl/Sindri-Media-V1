        ...
        CREATE  PROCEDURE `performance_articles2_card_day_dbt_13`()
        BEGIN
        ...
        left join prod.events e on e.postid=ldp.id   and e.siteid = ldp.siteid and e.date between DATE_SUB(NOW(), INTERVAL 8 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY) and e.siteid = 13
        ...
        and e.date  between DATE_SUB(NOW(), INTERVAL 15 DAY) and DATE_SUB(NOW(), INTERVAL  8 DAY) and e.siteid = 13
        and e.Event_Action='Next Click'
        group by 1,2,3,5
        ),
        ...
        where l.siteid = 13 -- and l.Event_Action is not null
        union 
        ...
        where p.siteid = 13 -- and -- l.Event_Action is not null
        ),
