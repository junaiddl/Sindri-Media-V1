...
CREATE PROCEDURE `Traffic_pageview_month_dbt_11`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        pageview_first AS (
            ...
            WHERE siteid = 11 AND date between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY)
        ...
        newsletter_data_first AS (
            SELECT 
                ...
                AND n.referrer_name IN ( ''NL Red'', ''NL Fag'' )
        ...
        cms_groups_without_others_second as(
            ...
            SELECT siteid, id, date, "inspiration" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*inspiration.*"
            UNION ALL
        ...
        cms_groups_without_others_third as(
            SELECT siteid, id, date, "skolepolitik" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*skolepolitik.*"
            UNION ALL
            
            SELECT siteid, id, date, "DLF" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*DLF.*"
            UNION ALL
            
            SELECT siteid, id, date, "skoleledelse" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*skoleledelse.*"
            UNION ALL
            
            SELECT siteid, id, date, "psykisk arbejdsmiljø" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*psykisk arbejdsmiljø.*"
            UNION ALL
            
            SELECT siteid, id, date, "forskning" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*forskning.*"
        ),
        cms_groups_third as (
            SELECT a.siteid, a.id, a.date, ''Others'' as tags, a.link
            FROM cms_data a
        ...
        cms_groups_without_others_fourth as(
        ...
        cms_groups_fourth as (
            SELECT a.siteid, a.id, a.date, ''Others'' as tags, a.link
            FROM cms_data a
            LEFT JOIN cms_groups_without_others_fourth b
            ON a.siteid = b.siteid AND a.id = b.id
            WHERE b.siteid IS NULL

            UNION ALL

            SELECT * FROM cms_groups_without_others_fourth
        ),
        final_transformed_fourth AS (
            ...
            LEFT JOIN cms_groups_fourth p ON s.siteid = p.siteid AND s.modifyurl = p.link
            WHERE p.id IS NOT NULL
            and s.siteid = 11 
        ),
        ...
        grouped_visits_fourth AS (
             SELECT
                ...
                n.visits AS visits,
                s.tags
            FROM newsletter_data_fourth n
            LEFT JOIN cms_groups_fourth s
                    ON n.post_id = s.id AND s.siteid = n.site_id
        ),
        total_data_fourth As(
            ...
            UNION
            select * from grouped_visits_fourth
        ),
