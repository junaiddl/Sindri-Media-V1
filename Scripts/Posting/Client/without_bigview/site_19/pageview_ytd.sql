...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_19`(
    IN site INT,
...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        ...
        WHERE realreferrer IN ( ''Forside'',''Search'',''LinkedIn'', ''Newsletter'' ) and siteid = 19'
    );
    
        SET @categories_sql_first = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Forbind mig'', ''Inspirer mig'', ''Hjælp mig med at forstå'', ''Giv mig en fordel'', ''Opdater mig'', ''Underhold mig'' ) and siteid = 19'
        );
    
        SET @categories_sql_second = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Nyhed'', ''Debat'', ''Event'', ''Magasinet med.'', ''Policy'' ) and siteid = 19'
        );
    
        SET @categories_sql_third = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Life science'', ''Lægemiddelindustrien i tal'', ''Sundhedspolitik'', ''Adgang til medicin'', ''Uddannelse & beskæftigelse'' ) and siteid = 19'
        );
    ...
    SET @sql_query = CONCAT('
        ...
        combinations_first AS (
            ...
            ORDER BY FIELD(r.realreferrer, ''Forside'',''Search'',''LinkedIn'', ''Newsletter'', "Others")
        ),
        pageview_first AS (
            SELECT 
                siteid,
            ...
            WHERE siteid = 19 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
                AND r.realreferrer IN ( ''Forside'', ''Search'', ''LinkedIn'' )
        ...
        main_first AS (
            ...
            FROM prod.events
            WHERE siteid = 19 AND Event_Action = ''Frontpage'' 
        ...
        transformed_data_first AS (
            SELECT
                ...
                t_totals,
                CASE 
                    WHEN event_name LIKE ''://%'' THEN CONCAT(''https://www.lif.dk'', SUBSTRING(event_name, 5))
                    ELSE event_name
                END AS modifyurl
            FROM main_first
        ),
        ...
        newsletter_data_first AS (
            SELECT 
                ...
                AND n.referrer_name IN ( ''Newsletter'' )
        ...
        summed_data_first AS (
			...
			ORDER BY FIELD(c.realreferrer, ''Forside'',''Search'',''LinkedIn'', ''Newsletter'', "Others")
        ),
        percent_first AS (
            SELECT
                realreferrer,
        ...
        pivoted_data_first AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''Forbind mig'', ''Inspirer mig'', ''Hjælp mig med at forstå'', ''Giv mig en fordel'', ''Opdater mig'', ''Underhold mig'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''Forbind mig'', ''Inspirer mig'', ''Hjælp mig med at forstå'', ''Giv mig en fordel'', ''Opdater mig'', ''Underhold mig'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''Forbind mig'', ''Inspirer mig'', ''Hjælp mig med at forstå'', ''Giv mig en fordel'', ''Opdater mig'', ''Underhold mig'', "Others")), '']'') AS percentile
			FROM percent_first
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''LinkedIn'', ''Newsletter'', "Others")
		),
        final_first as(
			SELECT 	
        ...
        cms_groups_without_others_second as(
            SELECT siteid, id, date, "Nyhed" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Nyheder|Analyse|Pressemeddelelse.*"
            UNION ALL
            ...
            SELECT siteid, id, date, "Event" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Event.*"
            UNION ALL
            
            SELECT siteid, id, date, "Magasinet med." AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Magasinet med..*"
            UNION ALL
            
            SELECT siteid, id, date, "Policy" AS tags, link
            FROM cms_data
            WHERE Categories REGEXP ".*Policy|Høringssvar|Politisk notat.*"
        ),
        ...
        pivoted_data_second AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''Nyhed'', ''Debat'', ''Event'', ''Magasinet med.'', ''Policy'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''Nyhed'', ''Debat'', ''Event'', ''Magasinet med.'', ''Policy'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''Nyhed'', ''Debat'', ''Event'', ''Magasinet med.'', ''Policy'', "Others")), '']'') AS percentile
			FROM percent_second
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''LinkedIn'', ''Newsletter'', "Others")
		),
        ...
        cms_groups_without_others_third as(
            SELECT siteid, id, date, "Life science" AS tags, link
            FROM cms_data
            WHERE tags REGEXP ".*Life science|Forskning & innovation|Kliniske forsøg|EU.*"
            UNION ALL
            
            SELECT siteid, id, date, "Lægemiddelindustrien i tal" AS tags, link
            FROM cms_data
            WHERE tags REGEXP ".*Lægemiddelindustrien i tal.*"
            UNION ALL
            
            SELECT siteid, id, date, "Sundhedspolitik" AS tags, link
            FROM cms_data
            WHERE tags REGEXP ".*Sundhedspolitik|politik.*"
            UNION ALL
            
            SELECT siteid, id, date, "Adgang til medicin" AS tags, link
            FROM cms_data
            WHERE tags REGEXP ".*Adgang til medicin|Sygehusmedicin|Apoteksmedicin|Forsyning.*"
            UNION ALL
            
            SELECT siteid, id, date, "Uddannelse & beskæftigelse" AS tags, link
            FROM cms_data
            WHERE tags REGEXP ".*Uddannelse & beskæftigelse.*"
        ),
        cms_groups_third as (
            SELECT a.siteid, a.id, a.date, ''Others'' as tags, a.link
            FROM cms_data a
        ...
        pivoted_data_third AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''Life science'', ''Lægemiddelindustrien i tal'', ''Sundhedspolitik'', ''Adgang til medicin'', ''Uddannelse & beskæftigelse'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''Life science'', ''Lægemiddelindustrien i tal'', ''Sundhedspolitik'', ''Adgang til medicin'', ''Uddannelse & beskæftigelse'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''Life science'', ''Lægemiddelindustrien i tal'', ''Sundhedspolitik'', ''Adgang til medicin'', ''Uddannelse & beskæftigelse'', "Others")), '']'') AS percentile
			FROM percent_third
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''LinkedIn'', ''Newsletter'', "Others")
		),
