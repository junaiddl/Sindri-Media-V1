...
CREATE PROCEDURE `tactical_userneeds_pageviews_chart_month_dbt_19`()
BEGIN
	with cms_data as (
	...
	counts as (
		...
		last_30_days_article_published_without_others_second as(
                SELECT siteid, id, fdate, "Nyhed" AS tags
                FROM cms_data
                ...
                SELECT siteid, id, fdate, "Event" AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Event.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Magasinet med." AS tags
                FROM cms_data
                WHERE Categories REGEXP ".*Magasinet med..*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Policy" AS tags
                FROM cms_data
	...
	counts_second as (
		...
		categories_d_second as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Nyhed', 'Debat', 'Event', 'Magasinet med.', 'Policy', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Nyhed', 'Debat', 'Event', 'Magasinet med.', 'Policy', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Nyhed', 'Debat', 'Event', 'Magasinet med.', 'Policy', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Nyhed', 'Debat', 'Event', 'Magasinet med.', 'Policy', 'others' ) SEPARATOR ',') AS top_10
		...
		last_30_days_article_published_without_others_third as(
                SELECT siteid, id, fdate, "Life science" AS tags
                FROM cms_data
                WHERE tags REGEXP ".*Life science|Forskning & innovation|Kliniske forsøg|EU.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Lægemiddelindustrien i tal" AS tags
                FROM cms_data
                WHERE tags REGEXP ".*Lægemiddelindustrien i tal.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Sundhedspolitik" AS tags
                FROM cms_data
                WHERE tags REGEXP ".*Sundhedspolitik|politik.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Adgang til medicin" AS tags
                FROM cms_data
                WHERE tags REGEXP ".*Adgang til medicin|Sygehusmedicin|Apoteksmedicin|Forsyning.*"
                UNION ALL
                
                SELECT siteid, id, fdate, "Uddannelse & beskæftigelse" AS tags
                FROM cms_data
	...
	counts_third as (
		...
		categories_d_third as (
			select 
				...
				'Artikler ift. next click mål' as label,
                'Artikler grupperet ift hvordan de har performet på next click' as hint,
				GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Life science', 'Lægemiddelindustrien i tal', 'Sundhedspolitik', 'Adgang til medicin', 'Uddannelse & beskæftigelse', 'others' ) SEPARATOR ',') AS cateogires,
				GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Life science', 'Lægemiddelindustrien i tal', 'Sundhedspolitik', 'Adgang til medicin', 'Uddannelse & beskæftigelse', 'others' ) SEPARATOR ',') AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Life science', 'Lægemiddelindustrien i tal', 'Sundhedspolitik', 'Adgang til medicin', 'Uddannelse & beskæftigelse', 'others' ) SEPARATOR ',') AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Life science', 'Lægemiddelindustrien i tal', 'Sundhedspolitik', 'Adgang til medicin', 'Uddannelse & beskæftigelse', 'others' ) SEPARATOR ',') AS top_10
		from agg_data_third
		group by 1,2,3
		),
        categories_third as (
