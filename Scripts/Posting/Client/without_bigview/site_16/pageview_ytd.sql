...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_16`(
    IN site INT,
...
BEGIN
    ...
    SET @referrers_sql = CONCAT(
        ...
        WHERE realreferrer IN ( ''Forside'',''Search'',''Facebook'',''Newsletter'' ) and siteid = 16'
    );
    
        SET @categories_sql_first = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Opdater mig'',''Forbind mig'',''Hjælp mig med at forstå'',''Giv mig en fordel'',''Underhold mig'',''Inspirer mig'' ) and siteid = 16'
        );
    
        SET @categories_sql_second = CONCAT(
            'SELECT DISTINCT Categories FROM pre_stage.ref_value 
            WHERE Categories IN ( ''Løn og konkrete fordele'', ''Arbejdsmiljø'',''Det moderne arbejdsliv'',''Uddannelse'',''Grøn omstilling'',''Formanden har ordet'' ) and siteid = 16'
        );
    ...
    SET @sql_query = CONCAT('
        ...
        pageview_first AS (
            ...
            WHERE siteid = 16 AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
        ...
        transformed_data_first AS (
            SELECT
                ...
                t_totals,
                CASE 
                    WHEN event_name LIKE ''://%'' THEN CONCAT(''https://blikroer.dk/'', SUBSTRING(event_name, 5))
                    ELSE event_name
                END AS modifyurl
            FROM main_first
        ),
        ...
        cms_groups_without_others_second as(
            SELECT siteid, id, date, "Løn og konkrete fordele" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Løn.*"
            UNION ALL
            
            SELECT siteid, id, date, "Arbejdsmiljø" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Arbejdsmiljø.*"
            UNION ALL
            
            SELECT siteid, id, date, "Det moderne arbejdsliv" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Arbejdsliv.*"
            UNION ALL
            
            SELECT siteid, id, date, "Uddannelse" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Udannelse.*"
            UNION ALL
            
            SELECT siteid, id, date, "Grøn omstilling" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Grøn_omstilling.*"
            UNION ALL
            
            SELECT siteid, id, date, "Formanden har ordet" AS tags, link
            FROM cms_data
            WHERE Tags REGEXP ".*Formanden.*"
        ),
        ...
        pivoted_data_second AS (
			SELECT
				realreferrer,
				CONCAT(''['', GROUP_CONCAT(DISTINCT CONCAT(''"'', tag, ''"'') ORDER BY FIELD(tag, ''Løn og konkrete fordele'', ''Arbejdsmiljø'',''Det moderne arbejdsliv'',''Uddannelse'',''Grøn omstilling'',''Formanden har ordet'', "Others")), '']'') AS categories,
				GROUP_CONCAT(total_visits ORDER BY FIELD(tag, ''Løn og konkrete fordele'', ''Arbejdsmiljø'',''Det moderne arbejdsliv'',''Uddannelse'',''Grøn omstilling'',''Formanden har ordet'', "Others")) AS total_visits,
				CONCAT(''['', GROUP_CONCAT((percentile) ORDER BY FIELD(tag, ''Løn og konkrete fordele'', ''Arbejdsmiljø'',''Det moderne arbejdsliv'',''Uddannelse'',''Grøn omstilling'',''Formanden har ordet'', "Others")), '']'') AS percentile
			FROM percent_second
			GROUP BY realreferrer
             ORDER BY FIELD(realreferrer, ''Forside'',''Search'',''Facebook'',''Newsletter'', "Others")
		),
