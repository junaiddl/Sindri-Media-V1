        ...
        CREATE  PROCEDURE `performance_articles2_card_day_dbt_16`()
        BEGIN
        ...
        left join prod.events e on e.postid=ldp.id   and e.siteid = ldp.siteid and e.date between DATE_SUB(NOW(), INTERVAL 8 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY) and e.siteid = 16
        ...
        and e.date  between DATE_SUB(NOW(), INTERVAL 15 DAY) and DATE_SUB(NOW(), INTERVAL  8 DAY) and e.siteid = 16
        and e.Event_Action='Next Click'
        group by 1,2,3,5
        ),
        ...
        on e.postid=ldp.id   and e.siteid = ldp.siteid
        and e.date between DATE_SUB(NOW(), INTERVAL 8 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY) and e.siteid = 16
        group by 1,2,3
        ),
        ...
        where l.siteid = 16 -- and l.Event_Action is not null
        union 
        ...
        where p.siteid = 16 -- and -- l.Event_Action is not null
        ),
