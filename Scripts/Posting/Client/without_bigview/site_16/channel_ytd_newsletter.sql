...
CREATE PROCEDURE `Traffic_channel_ytd_newsletter_dbt_16`(
    IN site INT,
...
END
 ...
 AS Categories,
			...
			WHERE s.siteid = 16 and r.realreferrer = ',channel_label,'
...
''sektion'', coalesce(Categories, " "),
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,16) as siteid,
				ah.series,
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id","label": "ID"},{"field": "article","label": "ARTIKEL"},{"field": "category","label": "BRUGERBEHOV"},{"field": "tags","label": "FOKUSOMRÅDER", "hidden" : true},{"field": "date","label": "DATO"},{"field": "brugerbehov","label": "Kliks"}]
,'',''"rows":'',
							   tble.data
				   ,''},''
