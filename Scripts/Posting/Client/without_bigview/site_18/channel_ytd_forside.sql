...
CREATE DEFINER=`Site`@`%` PROCEDURE `Traffic_channel_forside_ytd_dbt_18`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
		...
		card_visits_last_year AS (
			...
			FROM prod.events
			where siteid = 18 and event_action = ''Frontpage''
			AND date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()) - 2, 1)
		 ...
		 final_card as(
		   SELECT
			  ...
			  ''Antal kliks på forsiden i perioden samt udvikling ift. forrige sammenlignelige periode'' as hint,
			   COALESCE(cur_v.siteid,18) as siteid,
			  cur_v.realreferrer,
		  ...
		  left JOIN card_visits_last_year last_v on cur_v.siteid  = last_v.siteid and cur_v.realreferrer = last_v.realreferrer
		...
		channels_table_real AS (
			...
			FROM prod.events
			WHERE
				siteid = 18 and event_action = ''Frontpage'' and
 ...
 AS Categories,
			...
			WHERE s.siteid = 18 and r.realreferrer = ''Forside''
            group by 1,2
...
''sektion'', coalesce(Categories, " "),
       ...
       visits_per_dayname AS (
		...
		Result AS (
			SELECT
				COALESCE(lastyear.siteid,18) as siteid,
				ah.series,
