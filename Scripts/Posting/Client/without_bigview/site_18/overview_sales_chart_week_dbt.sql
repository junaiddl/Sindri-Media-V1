    ...
    CREATE PROCEDURE `overview_sales_chart_week_dbt_18`()
    BEGIN
	...
	json_data_curr AS (
        ...
        WHERE
            ag.site_id = 18 AND ag.week <= CAST(WEEK(DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY), 3) as char)
        GROUP BY COALESCE(ao.SiteID, ag.site_id), label, hint
    )    
    SELECT
    CONCAT(
        '{',
        '"site":', COALESCE(p.SiteID, c.SiteID, 18), ',',
