    ...
    CREATE  PROCEDURE `overview_sales_card_dbt_18`()
        BEGIN
with curr_year as (
SELECT
    SiteID,
...
FROM
    prod.goals
  where site_id = 18 and date
between (SELECT MIN(date) AS MinimumDate
    FROM prod.pages
    WHERE siteid = 18 and YEAR(date) = YEAR(CURRENT_DATE()))  and  (SELECT MAX(date) AS MaximumDate
                  FROM prod.pages
                  WHERE siteid = 18 and YEAR(date) = YEAR(CURRENT_DATE()))
                  
 group by Site_Id
)
   SELECT JSON_OBJECT(
        'site', COALESCE(cy.siteid, ly.siteid, g.site_id, 18),
        'data', JSON_OBJECT(
            'label', 'Salg',
    ...
    FROM (SELECT 18 AS siteid) AS dummy
