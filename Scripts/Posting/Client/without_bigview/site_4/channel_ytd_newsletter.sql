...
CREATE PROCEDURE `Traffic_channel_ytd_newsletter_dbt_4`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        card_visits_last_month AS (
			...
			LEFT JOIN prod.newsletter n ON
				rc.realreferrer = n.referrer_name
				AND n.site_id = 4
				AND n.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()) - 2, 1)
					AND MAKEDATE(EXTRACT(YEAR FROM CURDATE()) - 1, 1) - INTERVAL 1 DAY
			GROUP BY 1,2
         ),
         final_card as(
           SELECT
 ...
 AS Categories,
			...
			WHERE s.siteid = 4 and r.realreferrer = ',channel_label,'
			AND visits >= 5
...
''sektion'', coalesce(userneeds, " "),
				   ...
				   ) ), ''[{"id": "4", "date": " ", "clicks": " " , "article": " ", "sektion": " ","BRUGERBEHOV": " ", "brugerbehov": 0}]'' )
                as data
				from site_j
        ),
        ...
        visits_per_dayname AS (
			...
			LEFT JOIN prod.traffic_channels tc ON ad.english_day = DAYNAME(tc.`date`)
				...
				AND siteid = 4 and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		...
		Result AS (
			SELECT
				COALESCE(lastweek.siteid,4) as siteid,
				ah.series,
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":[{"field": "id","label": "ID"},
