...
CREATE DEFINER=`Site`@`%` PROCEDURE `Traffic_channel_forside_ytd_dbt_4`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
		 ...
		 final_card as(
		   SELECT
			  ...
			  ''Antal kliks på forsiden i perioden samt udvikling ift. forrige sammenlignelige periode'' as hint,
			   COALESCE(cur_v.siteid,4) as siteid,
			  cur_v.realreferrer,
		...
		channels_table_real AS (
			...
			FROM prod.events
			WHERE
				siteid = 4 and event_action = ''Frontpage'' and
 ...
 AS Categories,
			...
			WHERE s.siteid = 4 and r.realreferrer = ''Forside''
			AND visits >= 5
            group by 1,2
...
''sektion'', coalesce(userneeds, " "),
       ...
       visits_per_dayname AS (
		...
		Result AS (
			SELECT
				COALESCE(lastyear.siteid,4) as siteid,
				ah.series,
