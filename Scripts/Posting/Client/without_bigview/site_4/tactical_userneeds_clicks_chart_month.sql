...
CREATE PROCEDURE `tactical_userneeds_clicks_chart_month_dbt_4`()
BEGIN
with 
    last_ytd_article_published as(
        SELECT siteid as siteid,id, date as fdate   FROM prod.site_archive_post
        where 
        date between date_sub(current_date(),interval 1 month)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)  
        and siteid = 4
