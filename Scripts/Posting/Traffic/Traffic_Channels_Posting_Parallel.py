...
def  get_output(inputs, barchart_name, channel_procedure, forside_channel_procedure, currentSite):
    ...
    for vals in vals_list:
        clean_val = vals.strip("'")  # Remove extra single quotes if they exist
        print(f"'{clean_val}'")  # Print each value wrapped in a single pair of quotes
        print("\n\n\n")


    # Function to be run in parallel for each val in vals_list
    def process_val(val):
        print(f"Processing {val} in thread: {threading.current_thread().name}")
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # Submit all vals in vals_list to be processed in parallel
        futures = [executor.submit(process_val, val) for val in vals_list]
        
        # Collect the results as they complete
        for future in concurrent.futures.as_completed(futures):
            channels = future.result()
            if channels:
                key_val = channels['label']
                temp_json[key_val] = channels
    
    for vals in vals_list:
        clean_val = vals.strip("'")  # Remove extra single quotes if they exist
        print(f'{clean_val}')
        print(temp_json[f'{clean_val}'])
        main_json['data']['channels'].append(temp_json[f'{clean_val}'])
...
def start_execution(sites):
    try:
        # truncate_data_quality(sites)
        print(f"Started posting for Site ID: {sites}")
        
        with concurrent.futures.ThreadPoolExecutor() as executor:
            # Submit line_charts tasks to the executor
            day_chart_future = executor.submit(line_charts, f'Traffic_dynamic_linequery_day_dbt_{sites}', sites, linechart_label, linechart_hint)
            week_chart_future = executor.submit(line_charts, f'Traffic_dynamic_linequery_week_dbt_{sites}', sites, linechart_label, linechart_hint)
            month_chart_future = executor.submit(line_charts, f'Traffic_dynamic_linequery_month_dbt_{sites}', sites, linechart_label, linechart_hint)
            
            # # # Submit working tasks to the executor
            week_result_future = executor.submit(working, f'Traffic_pageview_week_dbt_{sites}', f'Traffic_channel_week_dbt_{sites}', f'Traffic_channel_forside_week_dbt_{sites}', sites)
            month_result_future = executor.submit(working, f'Traffic_pageview_month_dbt_{sites}', f'Traffic_channel_month_dbt_{sites}', f'Traffic_channel_forside_month_dbt_{sites}', sites)
            ytd_result_future = executor.submit(working, f'Traffic_pageview_ytd_dbt_{sites}', f'Traffic_channel_ytd_dbt_{sites}', f'Traffic_channel_forside_ytd_dbt_{sites}', sites)

            # Wait for all tasks to complete
            day_chart = day_chart_future.result()
            # day_chart['site'] = 2
            week_chart = week_chart_future.result()
            # week_chart['site'] = 2
            month_chart = month_chart_future.result()
            # month_chart['site'] = 2
            
            week_result = week_result_future.result()
            print(week_result)
            # week_result['site'] = 2
            month_result = month_result_future.result()
            # month_result['site'] = 2
            ytd_result = ytd_result_future.result()
            # ytd_result['site'] = 2

        # Now that all tasks are completed, we can proceed
