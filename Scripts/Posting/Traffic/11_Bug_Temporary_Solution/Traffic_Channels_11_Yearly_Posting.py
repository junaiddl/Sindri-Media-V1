...
def get_database_connection():
    try:
        ...
        cursor.execute("SET SESSION MAX_EXECUTION_TIME=6000")
...
def barchart_query(procedure_name, currentSite, label, hint, realreferrer, tags_1,tags_2):
    ...
    query = f"call prod.final_json_ytd_11();"
...
# def post_data(day_chart,week_chart,month_chart,week_result,month_result):
def post_data(ytd_result):
...
def start_execution(id): 
    for sites in [id]:
        ...
        try:
            ...
            # post_data(day_chart,week_chart,month_chart,week_result,month_result)
            post_data(ytd_result)
