...
linechart_label = 'BesÃ¸g'
linechart_hint = 'BesÃ¸g opdelt pÃ¥ trafikkanaler'


card_label = "BesÃ¸g"
card_hint = "Antal besÃ¸g fra valgte trafikkanal samt udvikling ift. forrige sammenlignelige periode"
chartbar_label = "Ugerytme"
chartbar_hint = "Fordeling af besÃ¸g pÃ¥ ugedage for den valgte trafikkanal"
chartbar_name = "andel besÃ¸g"
chartline_label = "DÃ¸gnrytme"
chartline_hint = "Fordeling af besÃ¸g pÃ¥ timer i dÃ¸gnet for hhv. hverdage og weekend for den valgte trafikkanal"
card_forside_label = "Kliks pÃ¥ forsiden"
card_forside_hint = "Antal kliks pÃ¥ forsiden i perioden samt udvikling ift. forrige sammenlignelige periode"
chartline_forside_label = "DÃ¸gnrytme"
chartline_forside_hint = "Fordeling af kliks pÃ¥ forsiden pÃ¥ timer i dÃ¸gnet for hhv. hverdage og weekend."
chartbar_name_forside = "andel besÃ¸g"
chartbar_label_forside = "Ugerytme"
chartbar_hint_forside = "Fordeling af kliks pÃ¥ forsiden pÃ¥ ugedage"




def  get_output(inputs, barchart_name, channel_procedure, forside_channel_procedure, newsletter_channel_procedure, currentSite):
    label =  '''Hvilken type indhold skaber engagement pÃ¥ tvÃ¦rs af trafikkanaler?'''
    hint = '''Andelen af trafik til indgangssider for de forskellige trafikkanaler grupperet ift. indgangssidernes tags/kategorier. For Forside er artikler, der klikkes pÃ¥ fra forsiden i stedet for indgangssider.'''
    ...
    if len(inputs['pageview_input']) >= 1:
        for key, value in inputs['pageview_input'].items():
            if key == "default_d":
                ...
                r_tags_one = "''Opdater mig'',''Forbind mig'',''HjÃ¦lp mig med at forstÃ¥'',''Giv mig en fordel'',''Underhold mig'',''Inspirer mig''"
    ...
    for val in vals_list:
        ...
        # Remove surrounding triple single quotes if they exist
        cleaned_val = val.strip("'")
        if cleaned_val == "Newsletter" or cleaned_val.startswith("NL"):
            channels = channel_query(newsletter_channel_procedure,
                                    currentSite,
                                    val,
                                    card_label,
                                    card_hint,
...
def working(barchart_procedure, channel_procedure, forside_channel_procedure, newsletter_channel_procedure, currentSite):
   
    print('start')
    input_values = fetch_inputs(12,'month')
    print(input_values)
    result= get_output(input_values, barchart_procedure, channel_procedure, forside_channel_procedure, newsletter_channel_procedure, currentSite)
...
def send_data_quality(id, traffic_status):
    # Get current date
    ...
    try:
        ...
        # Fetch cms_status and analytics_status from the database
        fetch_query = """
        SELECT COALESCE(MAX(traffic_ingestion_status), 0)  
        FROM prod.posting_status 
        WHERE siteid = %s AND posting_date = %s
        """
        cursor.execute(fetch_query, (id, current_date))
        result = cursor.fetchone()

        # Determine traffic_status based on cms_status and analytics_status
        if result:
            traffic_ingestion_status = result[0]
            if traffic_ingestion_status == 1:
                traffic_status = 1
            else:
                traffic_status = 0
        else:
            # Default to failure if no data exists for the site
            traffic_status = 0

        # Insert or update the traffic_status in prod.posting_status
        insert_query = """
        INSERT INTO prod.posting_status (siteid, posting_date, traffic_status)
        VALUES (%s, %s, %s)
        """
        cursor.execute(insert_query, (id, current_date, traffic_status))
        connection.commit()

        print(f"Updated traffic_status for ID {id} on {current_date} to {traffic_status}")

    except mysql.connector.Error as db_err:
        print(f"Failed to update traffic_status: {db_err}")
...
def truncate_data_quality(site):
    ...
    sql_query = f"""DELETE FROM prod.posting_status WHERE posting_date = '{current_date}' AND swagger_status IS NULL AND analytics_status IS NULL AND cms_status IS NULL AND traffic_ingestion_status IS NULL AND siteid = {site};"""
...
def start_execution(id): 
    for sites in [id]:
        ...
        try:
            ...
            week_result = working(f'Traffic_pageview_week_dbt_{sites}', f'Traffic_channel_week_dbt_{sites}', f'Traffic_channel_forside_week_dbt_{sites}', f'Traffic_channel_week_newsletter_dbt_{sites}', sites)
            month_result = working(f'Traffic_pageview_month_dbt_{sites}',f'Traffic_channel_month_dbt_{sites}',f'Traffic_channel_forside_month_dbt_{sites}', f'Traffic_channel_month_newsletter_dbt_{sites}', sites)
            ytd_result = working(f'Traffic_pageview_ytd_dbt_{sites}', f'Traffic_channel_ytd_dbt_{sites}', f'Traffic_channel_forside_ytd_dbt_{sites}', f'Traffic_channel_ytd_newsletter_dbt_{sites}', sites)
