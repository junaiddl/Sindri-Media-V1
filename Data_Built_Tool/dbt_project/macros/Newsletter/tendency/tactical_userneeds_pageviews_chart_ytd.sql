{% macro tactical_userneeds_pageviews_chart_ytd_dbt_newsletter() %}
...
CREATE PROCEDURE `tactical_userneeds_pageviews_chart_ytd_dbt_{{site}}`()
BEGIN
    with cms_data as (
	...
	agg_next_click_per_article_without_others{{ count[i] }} as(
	...
	last_30_days_article_published{{ count[i] }} as (
       SELECT a.siteid, a.id, a.fdate, 'others' as Tags
                FROM cms_data a
                LEFT JOIN agg_next_click_per_article_without_others{{ count[i] }} b
                ON a.siteid = b.siteid AND a.id = b.id
                WHERE b.siteid IS NULL

                UNION ALL

                SELECT * FROM agg_next_click_per_article_without_others{{ count[i] }}
				),
	...
	agg_next_click_per_article{{ count[i] }} as 
      (
		...
		left join prod.events  a on a.postid=e.id and a.siteid = e.siteid and a.event_action = '{{event_name}}'
		where    e.siteid = {{site}}
		group by 1,2,3
		),
