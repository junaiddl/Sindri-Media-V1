{% macro newsletter_set_articlestats_dbt() %}   

{% set site = var('site')%}
{% set event = var('event_action')%}

CREATE  PROCEDURE `set_articlestats_dbt_{{site}}`()
BEGIN
...
pages_d AS (
    ...
    FROM
        prod.pages p
    WHERE
        p.siteid = {{site}} and PostID is not null
...
SELECT
    JSON_OBJECT(
        ...
        'clicks', COALESCE(CAST(clicks AS SIGNED), 0)
