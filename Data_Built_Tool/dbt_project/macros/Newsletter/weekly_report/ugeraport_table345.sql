...
{% macro ugerapport_table345_dbt_newsletter() %}
...
combined AS (
    SELECT 
		...
		COALESCE(c.total_cta, 0) AS gns_cta
...
table4 AS (
    SELECT
        ROUND(SUM(CASE WHEN c.gns_cta >= (SELECT min_cta FROM goals) OR c.pageviews >= (SELECT min_pageviews FROM goals) THEN 1 ELSE 0 END) / COUNT(*) * 100, 1) AS over_et_maal,
