{% macro newsletter_bigview_header_yesterday_dbt() %}
    {% set siteid = var('site')%}
    {% set event_action = var('event_action')%}

CREATE PROCEDURE `bigview_header_yesterday_dbt_{{siteid}}`()
BEGIN
...
unique_pageviews_card AS (
    SELECT 4 as siteid, y.value, b.value_day_before
...
unique_pageviews_chart as (
    ...
    where date BETWEEN DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY) and siteid = 4
    group by 1,2
    ),
...
visits_chart as (
    ...
    FROM prod.daily_totals
    where
    date BETWEEN DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY) and siteid = 4
    group by 1,2
    ),
...
next_click_chart AS(
    SELECT 4 AS siteid, e.date AS date, SUM(e.data) AS next_click_data
    FROM events_chart e
    GROUP BY 1,2
    ORDER BY e.date ASC
),
