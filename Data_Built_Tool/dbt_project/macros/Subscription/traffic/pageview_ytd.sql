{% macro pageview_ytd_dbt_subscription() %}
...
CREATE PROCEDURE `Traffic_pageview_ytd_dbt_{{siteid}}`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        pageview{{ count[i] }} AS (
            ...
            WHERE siteid = {{siteid}} AND date BETWEEN  MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
        ...
        final_transformed{{ count[i] }} AS (
            ...
            CASE
                {%- for j in range(dropdown_val[dropdown_mapping[dropdown_titles[i]]] | length) %}
				WHEN {{ dropdown_mapping[dropdown_titles[i]] }} REGEXP ".*{{dropdown_val[dropdown_mapping[dropdown_titles[i]]][j]}}.*" THEN "{{ dropdown_val_m[dropdown_mapping[dropdown_titles[i]]][j] }}"
                {%- endfor %}
                ELSE ''others''
			END AS tags
