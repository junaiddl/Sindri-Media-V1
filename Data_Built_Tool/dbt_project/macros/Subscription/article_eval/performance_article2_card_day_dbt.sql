{% macro subscription_performance_articles2_card_day_dbt() %}        
        {% set site = var('site') %}
        ...
        left join prod.events e on e.postid=ldp.id 
          and e.siteid = ldp.siteid and e.date between DATE_SUB(NOW(), INTERVAL 8 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY) and e.siteid = {{site}}
