{% macro string_tactical_category_clicks_chart_ytd_dbt_nextclick() %}

{% set site = var('site') %}
{% set tag_filters_exclude = var('condition') %}


CREATE PROCEDURE `tactical_category_clicks_chart_ytd_dbt_{{site}}`()
BEGIN
    with last_ytd_article_published as (
        SELECT siteid as siteid,id,date as fdate   FROM prod.site_archive_post_string
        where 
        date  between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)  and siteid ={{site}}
