{% macro string_tactical_pageviews_card_ytd_dbt_nextclick() %}

{% set site = var('site') %}

{% set tag_filters_exclude = var('condition') %}

CREATE PROCEDURE `tactical_pageviews_card_ytd_dbt_{{site}}`()
BEGIN
    ...
    last_ytd_before_article_published as (
        SELECT siteid as siteid,id   FROM prod.site_archive_post_string
        where 
        DATE_SUB(MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1), INTERVAL 1 Year)  and  DATE_SUB(DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY),INTERVAL 1 Year) and siteid = {{site}}
        {% if tag_filters_exclude.status %}
        {{tag_filters_exclude.query}}
        {% endif %}
    ),
