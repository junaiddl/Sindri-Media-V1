{% macro string_next_click_performance_articles1_tables_ytd_dbt() %}   
        ...
        CREATE DEFINER=`Site`@`%` PROCEDURE `performance_articles1_tables_ytd_dbt_{{site}}`()
        BEGIN
        ...
        last_ytd_hits as (
            ...
            FROM   
                last_ytd_post ldp
                LEFT JOIN prod.events_string e ON e.postid = ldp.id AND e.siteid = ldp.siteid AND e.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY) AND e.siteid = {{site}} AND e.Event_Action =  '{{event}}'
        ...
        last_ytd_pageview as (
            ...
            FROM 
                last_ytd_post ldp
                LEFT JOIN prod.pages_string e ON e.postid = ldp.id AND e.siteid = ldp.siteid AND e.date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY) AND e.siteid = {{site}}
            GROUP BY 1, 2, 3, 5, 7, 8, 9,10
            {% if col_status %}
                    ,ldp.{{col_name}}
                {% endif %}
