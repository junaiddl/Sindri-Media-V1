{% macro string_next_click_performance_articles1_card_day_dbt() %}		
		{% set site = var('site') %}
		...
		last_7days_pageview as(
			...
			on e.postid=ldp.id and ldp.siteid=e.siteid
			and e.date between DATE_SUB(NOW(), INTERVAL 8 DAY) and DATE_SUB(NOW(), INTERVAL  1 DAY)  and ldp.siteid = {{site}}
			group by 1,2,3
		),
