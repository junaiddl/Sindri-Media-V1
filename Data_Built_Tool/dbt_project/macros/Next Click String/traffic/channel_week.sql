{% macro string_channel_week_dbt_nextclick() %}
...
CREATE PROCEDURE `Traffic_channel_week_dbt_{{siteid}}`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
		...
		site_j AS (
            ...
            group by 1,2,3,4,5,6,{% for i in range(7, 7+total_columns) %} {{i}} {%- if not loop.last %}, {% endif %}{% endfor %}
		),
        table_data AS (
			select 
