{% macro string_channel_forside_month_dbt_nextclick() %}
...
CREATE DEFINER=`Site`@`%` PROCEDURE `Traffic_channel_forside_month_dbt_{{siteid}}`(
    IN site INT,
    ...
    SET @sql_query = CONCAT('
		...
		card_visits_last_year AS (
		 ...
		 final_card as(
			...
			COALESCE(cur_v.siteid, {{siteid}}) AS siteid,
			...
			FROM (SELECT {{siteid}} AS siteid, ''Forside'' AS realreferrer, 0 AS visits) dummy
		...
		channels_table_real AS (
			...
			FROM prod.events_string
			WHERE
				siteid = {{siteid}} and event_action = ''Frontpage'' and
		...
		site_j AS (
			...
			WHERE s.siteid = {{siteid}} and r.realreferrer = ''Forside''
       ...
       visits_per_dayname AS (
		...
		Result AS (
			SELECT
				COALESCE(lastyear.siteid,{{siteid}}) as siteid,
				ah.series,
