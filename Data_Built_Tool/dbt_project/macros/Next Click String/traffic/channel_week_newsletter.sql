{% macro string_channel_week_dbt_nextclick_news() %}
...
{% set entry_pages_json_modified = var('Traffic').entry_pages_json | replace('"Besøg"', '"Kliks"') %}
...
CREATE PROCEDURE `Traffic_channel_week_newsletter_dbt_{{siteid}}`(
    IN site INT,
    ...
    SET @sql_query = CONCAT('
		...
		WeekVisits_chartbar AS (
			...
			LEFT JOIN prod.traffic_channels_string tc ON ad.english_day = DAYNAME(tc.`date`)
				...
				AND siteid = {{siteid}} and realreferrer = ',channel_label,'
			GROUP BY 1,2,3,5
		   ...
		   ''{"label":"'' ',channel_label,'  ''",'', 
				''"data": {'',
					''"card1": { '',
						   ...
						   '',"value":'', c1.value,
					 ''},'',
					  ''"entrypages":{'',''"columns":{{entry_pages_json_modified}},'',''"rows":'',
							   tble.data
				   ,''},''
