{% macro string_pageview_week_dbt_nextclick() %}
...
CREATE PROCEDURE `Traffic_pageview_week_dbt_{{siteid}}`(
    IN site INT,
...
BEGIN
    ...
    SET @sql_query = CONCAT('
        ...
        pageview{{ count[i] }} AS (
            ...
            WHERE siteid = {{siteid}} AND date BETWEEN DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY)
