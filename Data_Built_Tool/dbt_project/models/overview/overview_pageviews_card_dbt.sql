...
{% if event== "Next Click" and stringid == false %}
    {{ next_click_overview_pageviews_card_dbt()}}
{% elif event == 'Subscription' and stringid == false %}
    {{ subscription_overview_pageviews_card_dbt() }}
{% elif event == 'Newsletter' and stringid == false %}
    {{ newsletter_overview_pageviews_card_dbt() }}
{% elif event == 'Sales' and stringid == false %}
    {{ sales_overview_pageviews_card_dbt() }}
{% elif event == 'Next Click' and stringid == true %}
    {{ string_next_click_overview_pageviews_card_dbt() }}
{% endif %}
