...
--   FROM prod.events
--   WHERE event_action = 'next_click'
--   GROUP BY siteid, date, postid
-- ),
-- filtered_goals AS (
--   SELECT Site_ID, Date, Min_CTA
--   FROM prod.goals
-- ),
-- page_event_data AS (
--   SELECT 
...
--     SUM(CASE 
--           WHEN actual_cta IS NOT NULL AND actual_cta < CAST(Min_CTA AS DECIMAL(10,4)) 
--           THEN 1 ELSE 0 
...
--     ELSE 'False'
--   END AS more_than_50pct_below_min_cta
-- FROM evaluation;
