...
--   SELECT 
--     CURDATE() - INTERVAL (WEEKDAY(CURDATE()) + 1) DAY AS end_date,  -- Sunday
--     CURDATE() - INTERVAL (WEEKDAY(CURDATE()) + 7) DAY AS start_date  -- Monday last week
-- ),
-- filtered_articles AS (
--   SELECT id AS postid
--   FROM prod.site_archive_post
--   WHERE userneeds = 'Hjælp mig med at forstå'
-- ),
-- weekly_page_data AS (
--   SELECT 
--     p.siteid,
...
evaluation AS (
  SELECT
    COUNT(*) AS total_articles,
    SUM(CASE 
          WHEN CAST(unique_pageviews AS UNSIGNED) < CAST(Min_pageviews AS UNSIGNED) 
          THEN 1 ELSE 0 
        END) AS below_goal_count
  FROM weekly_page_data
)
SELECT
  CASE 
    WHEN below_goal_count > total_articles * 0.55 THEN 'True'
