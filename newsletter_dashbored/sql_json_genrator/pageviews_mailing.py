...
def pageviews_mailing(list_id):

    pageviews_month_query = f"""
WITH who_is_sub AS (
    SELECT DISTINCT 
        created_at,
        site_id,
...
WITH who_is_sub AS (
    ...
    FROM pre_stage.whoissubscribed_data  
    WHERE created_at = CURDATE() - INTERVAL 2 DAY
      AND site_id = 14 and list_id = {list_id}
),
...
"""
    return pageviews_month_query, pageviews_ytd_query