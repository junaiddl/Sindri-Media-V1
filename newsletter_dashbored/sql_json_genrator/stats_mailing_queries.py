...
def stats_mailing_queries(list_id):
#     stats_day_query  = f"""
#     WITH subscriber_clicks AS (
#     SELECT siteid, subscriber_token, COUNT(*) AS total_clicks
#     FROM pre_stage.click_events
#     WHERE siteid = 14
#     AND created_date BETWEEN DATE_SUB(NOW(), INTERVAL 8 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY)
#     and mailing_list = {list_id}
...
#     WHERE c.siteid = 14 and c.mailing_list = {list_id}
# ),
# cleaned AS (
...
WITH subscriber_clicks AS (
    ...
    FROM pre_stage.click_events
    WHERE siteid = 14
    AND created_date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
    AND mailing_list = {list_id}
    GROUP BY siteid, subscriber_token
),
ranked AS (
...
select * from final_json;
"""
    return  stats_month_query, stats_ytd_query