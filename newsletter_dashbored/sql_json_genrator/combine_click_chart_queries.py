...
click_counts AS (
	...
	where
     created_date BETWEEN DATE_SUB(NOW(), INTERVAL 31 DAY) AND DATE_SUB(NOW(), INTERVAL 1 DAY)
...
click_counts AS (
    ...
    FROM pre_stage.click_events
	where
     created_date BETWEEN MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1) AND DATE_SUB(CAST(NOW() AS DATE), INTERVAL 1 DAY)
    GROUP BY siteid, subscriber_token
),
...
GROUP BY siteid;
    """
    return click_chart_month, click_chart_ytd
